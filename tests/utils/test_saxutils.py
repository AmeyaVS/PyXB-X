...
class TestInScopeNames(unittest.TestCase):
    def show(self, node):
        xmlns_map = pyxb.namespace.NamespaceContext.GetNodeContext(
            node
        ).inScopeNamespaces()
    ...
    def stripUndeclaredNamespaces(self, xmlns_map):
        ...
        self.assertEqual(
            pyxb.namespace.XMLNamespaces.uri(), xmlns_map.pop("xmlns").uri()
        )
        return xmlns_map
...
<book xmlns='urn:loc.gov:books'
    ...
    <notes>
      ...
      <p>another graf without namespace change</p>
        ...
        saxer = make_parser(
            element_state_constructor=StateTest,
            location_base="test_6_2_2",
            fallback_namespace=BogusNamespace,
        )
        ...
        self.assertEqual(
            book.namespaceContext().inScopeNamespaces().get("isbn"), isbn_ns
        )

        title = StateTest.StateSequence[2]
        xmlns_map = self.stripUndeclaredNamespaces(
            title.namespaceContext().inScopeNamespaces()
        )
        ...
        xmlns_map = self.stripUndeclaredNamespaces(
            p.namespaceContext().inScopeNamespaces()
        )
        ...
        xmlns_map = self.stripUndeclaredNamespaces(
            x.namespaceContext().inScopeNamespaces()
        )
   ...
   <tr>
     ...
     <td>
       <details xmlns=""><class>Bitter</class><hop>Fuggles</hop>
        ...
        xmlns_map = self.stripUndeclaredNamespaces(
            Beers.namespaceContext().inScopeNamespaces()
        )
        ...
        xmlns_map = self.stripUndeclaredNamespaces(
            table.namespaceContext().inScopeNamespaces()
        )
        self.assertEqual(1, len(xmlns_map))
        self.assertEqual("http://www.w3.org/1999/xhtml", xmlns_map[None].uri())
        brandName = StateTest.StateSequence[9]
        xmlns_map = self.stripUndeclaredNamespaces(
            brandName.namespaceContext().inScopeNamespaces()
        )
