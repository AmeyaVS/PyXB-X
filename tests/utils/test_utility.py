...
class BasicTest(unittest.TestCase):
    ...
    cases = (
        (r'"1\x042"', "1\0042"),  # expanded octal
        (
            r'"1\x042"',
            "1\0042",
        ),  # expanded octal (single quotes do not affect escaping)
        ...
        (
            "\"1'\\n'2\"",
            "1'\n'2",
        ),  # expanded newline to escape sequence (single quotes)
        ...
        (
            qu('"\\u0022"'),
            six.u("\u0022"),
        ),  # single quotes don't change that expanded unicode works
        (qu('"\\u0022"'), six.u(r"\u0022")),  # raw has no effect on unicode escapes
        (qu('"'), six.u("'")),  # unicode single quote works
        (
            qu('"\\u00220\\u0022"'),
            six.u('"\u0030"'),
        ),  # unicode with double quotes works
    )
    ...
    def testDFSOrder1(self):
        ...
        for source in order:
            for target in graph.edgeMap().get(source, []):
                self.assertTrue(
                    (target in walked)
                    or (graph.sccForNode(source) == graph.sccForNode(target)),
                    "%s -> %s not satisfied, seen" % (source, target),
                )
            walked.add(source)
...
class TestUniqueIdentifier(unittest.TestCase):
    ...
    def testRepr(self):
        ...
        rep = repr(u1)
        self.assertEqual(
            "pyxb.utils.utility.UniqueIdentifier(%s)" % (repr(u1.uid()),), repr(u1)
        )
...
class TestGetMatchingFiles(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if self.__haveSymlink:
            os.symlink(
                os.path.join(self.__directory, "d2"),
                os.path.join(self.__directory, "d1", "d11", "l2"),
            )
    ...
    def testPattern(self):
        files = set(
            self._stripPath(GetMatchingFiles(self._formPath("d1"), self.__WXS_re))
        )
        self.assertEqual(files, set(["d1/f1b.wxs", "d1/f1a.wxs"]))
        files = set(
            self._stripPath(GetMatchingFiles(self._formPath("d1"), self.__NoExt_re))
        )
        self.assertEqual(files, set(["d1/f1c"]))
        files = set(
            self._stripPath(GetMatchingFiles(self._formPath("d1", "d2"), self.__WXS_re))
        )
    ...
    def testD1D2(self):
        ...
        self.assertEqual(
            files, set(["d1/f1a.wxs", "d1/f1b.wxs", "d1/f1c", "d2/f2a.wxs", "d2/f2b"])
        )
    ...
    def testLink(self):
        if self.__haveSymlink:
            files = set(
                self._stripPath(
                    GetMatchingFiles(self._formPath(os.path.join("d1", "d11", "l2")))
                )
            )
            self.assertEqual(files, set(["d1/d11/l2/f2a.wxs", "d1/d11/l2/f2b"]))

    def testDefault(self):
        kw = {"default_path": self._formPath("d1"), "default_path_wildcard": "+"}
        files = set(
            self._stripPath(GetMatchingFiles(self._formPath("+"), self.__WXS_re, **kw))
        )
        self.assertEqual(files, set(["d1/f1b.wxs", "d1/f1a.wxs"]))
        files = set(
            self._stripPath(
                GetMatchingFiles(self._formPath("d2", "+"), self.__WXS_re, **kw)
            )
        )
        self.assertEqual(files, set(["d1/f1a.wxs", "d1/f1b.wxs", "d2/f2a.wxs"]))
        files = set(
    ...
    def testPrefixPattern(self):
        kw = {"prefix_pattern": "&", "prefix_substituend": self.__directory}
        # Note: &/d1 => /d1 so it does not count
        files = set(
            self._stripPath(
                GetMatchingFiles(
                    os.pathsep.join([os.path.join("&", "d1"), "&d2"]),
                    self.__NoExt_re,
                    **kw
                )
            )
    ...
    def testFiles(self):
        files = set(
            self._stripPath(
                GetMatchingFiles(
                    self._formPath("d1", os.path.join("d2", "f2b")), self.__NoExt_re
                )
            )
        )
