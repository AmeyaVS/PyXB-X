...
schema_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../schemas/test-ctd-attr.xsd")
)
...
class TestCTD(unittest.TestCase):
    ...
    def testDeconflict(self):
        ...
        self.assertTrue(
            issubclass(structure_, pyxb.binding.basis.complexTypeDefinition)
        )
        self.assertTrue(
            pyxb.binding.basis.complexTypeDefinition._CT_ELEMENT_ONLY
            == structure_._ContentTypeTag
        )
    ...
    def testEmptyWithAttr(self):
        ...
        self.assertRaises(
            ...
            CreateFromDocument,
            '<emptyWithAttr xmlns="URN:testCTD"/>',
        )
        instance = CreateFromDocument(
            '<emptyWithAttr capitalized="false" xmlns="URN:testCTD"/>'
        )
        ...
        self.assertRaises(
            ...
            CreateFromDocument,
            '<emptyWithAttr capitalized="true" immutable="water"  xmlns="URN:testCTD"/>',
        )

        instance = CreateFromDocument(
            '<emptyWithAttr capitalized="true" language="hebrew"  xmlns="URN:testCTD"/>'
        )
        ...
        if sys.version_info[1] < 8:
            xmlt = six.u(
                '<tca:emptyWithAttr capitalized="false" language="french" xmlns:tca="URN:testCTD"/>'
            )
        else:
            xmlt = six.u(
                '<tca:emptyWithAttr language="french" capitalized="false" xmlns:tca="URN:testCTD"/>'
            )
        xmld = xmlt.encode("utf-8")
    ...
    def testRestrictedEWA(self):
        ...
        self.assertNotEqual(
            restrictedEWA_._AttributeMap["language"],
            emptyWithAttr_._AttributeMap["language"],
        )
        ...
        self.assertEqual(
            restrictedEWA_._AttributeMap["capitalized"],
            emptyWithAttr_._AttributeMap["capitalized"],
        )

    def testEmptyWithAttrGroups(self):
        xmlt = six.u(
            '<tca:emptyWithAttrGroups bMember1="xxx" xmlns:tca="URN:testCTD"/>'
        )
        xmld = xmlt.encode("utf-8")
        instance = CreateFromDocument(xmlt)
    ...
    def testUnrecognizedAttribute(self):
        xmlt = six.u(
            '<emptyWithAttr capitalized="false" garbage="what is this" xmlns="URN:testCTD"/>'
        )
        doc = pyxb.utils.domutils.StringToDOM(xmlt)
        self.assertRaises(
            UnrecognizedAttributeError, emptyWithAttr.createFromDOM, doc.documentElement
        )
