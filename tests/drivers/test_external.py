...
te_generator = pyxb.binding.generate.Generator(
...
st_generator = pyxb.binding.generate.Generator(
    allow_absent_module=True, generate_to_files=False
)
...
assert module_record is not None, "Unable to find %s in ns %s" % (
    st_generator.generationUID(),
    stns,
)
...
class TestExternal(unittest.TestCase):
    ...
    def testSharedTypes(self):
        self.assertEqual(
            word.typeDefinition()._ElementMap["from"].elementBinding().typeDefinition(),
            st.english,
        )
        self.assertEqual(
            word.typeDefinition()._ElementMap["to"].elementBinding().typeDefinition(),
            st.welsh,
        )
        one = st.english("one")
    ...
    def testWords(self):
        xmlt = six.u(
            '<ns1:word xmlns:ns1="URN:test-external"><from>one</from><to>un</to></ns1:word>'
        )
        xmld = xmlt.encode("utf-8")
    ...
    def testBadWords(self):
        xmlt = six.u(
            '<ns1:word xmlns:ns1="URN:test-external"><from>five</from><to>pump</to></ns1:word>'
        )
        xmld = xmlt.encode("utf-8")
    ...
    def testComplexShared(self):
        xmlt = six.u(
            '<ns1:lwords language="english" newlanguage="welsh" xmlns:ns1="URN:test-external">un</ns1:lwords>'
        )
    ...
    def testCrossedRestriction(self):
        ...
        # override it.
        self.assertEqual(
            st.extendedName._ElementMap["title"], restExtName._ElementMap["title"]
        )
        self.assertEqual(
            st.extendedName._ElementMap["forename"], restExtName._ElementMap["forename"]
        )
        self.assertEqual(
            st.extendedName._ElementMap["surname"], restExtName._ElementMap["surname"]
        )
        self.assertEqual(
            st.extendedName._ElementMap["generation"],
