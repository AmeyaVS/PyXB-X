...
schema_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../schemas/test-wildcard.xsd")
)
...
class TestIntensionalSet(unittest.TestCase):
    ...
    def testUnion_5(self):
        ...
        self.assertRaises(
            SchemaValidationError, UNION, [nc_not(ns1), set([None, ns2])]
        )  # 5.3
    ...
    def testIntersection_4(self):
        ...
        self.assertEqual(
            set([ns2, None]), ISECT([set([None, ns1, ns2]), set([ns2, None])])
        )
        self.assertEqual(set([]), ISECT([set([ns1]), set([ns2, None])]))
        self.assertEqual(
            set([]), ISECT([set([ns1]), set([ns2, ns1]), set([ns2, None])])
        )
        self.assertEqual(
            set([ns1]),
            ISECT([set([ns1, None]), set([None, ns2, ns1]), set([ns1, ns2])]),
        )
...
class TestWildcard(unittest.TestCase):
    ...
    def testMultiElement(self):
        ...
        for rep in range(0, 6):
            xmls = "<wrapper><first/><second/>%s</wrapper>" % (
                "".join(rep * ["<third/>"]),
            )
            doc = pyxb.utils.domutils.StringToDOM(xmls)
            if 3 >= rep:
                ...
                tested_overmax = True
                self.assertRaises(
                    UnrecognizedContentError, wrapper.createFromDOM, doc.documentElement
                )
    ...
    def testAttribute(self):
        # NB: Test on CTD, not element
        self.assertTrue(
            isinstance(wrapper_._AttributeWildcard, pyxb.binding.content.Wildcard)
        )
