...
schema_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../schemas/alt-po1.xsd")
)
...
Anytown, AS  12345-6789"""
    ...
    def testDOMElementSimpleContent(self):
        elt = (
            USAddress._ElementMap["street"]
            .elementBinding()
            .createFromDOM(self.street_dom)
        )
        self.assertEqual(ToDOM(elt).toxml("utf-8"), self.street_xmld)
    ...
    def testPurchaseOrder(self):
        po = purchaseOrder(
            shipTo=USAddress(name="Customer", street="95 Main St"),
