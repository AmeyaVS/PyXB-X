...
class TestAbstractElementError(unittest.TestCase):
    Good_xmlt = six.u(
        "<eCardinals><eConcCardCymru>un</eConcCardCymru><eConcCardEnglish>three</eConcCardEnglish></eCardinals>"
    )
    Good_xmld = Good_xmlt.encode("utf-8")

    Bad_xmlt = six.u(
        "<eCardinals><eConcCardCymru>un</eConcCardCymru><eAbstractCard>three</eAbstractCard></eCardinals>"
    )
    ...
    def testException(self):
        ...
        self.assertEqual(
            str(e), "Cannot instantiate abstract element eAbstractCard directly"
        )
    ...
    def testFromDocument(self):
        ...
        e = cm.exception
        self.assertFalse(
            (e.location is None) and (pyxb.XMLStyle_saxer == pyxb._XMLStyle)
        )
...
class TestSimpleContentAbsentError(unittest.TestCase):
    ...
    GoodSeq_xmlt = six.u(
        "<eCTwSCSequence><eCTwSC>1</eCTwSC><eCTwSC>2</eCTwSC></eCTwSCSequence>"
    )
    GoodSeq_xmld = GoodSeq_xmlt.encode("utf-8")
    BadSeq_xmlt = six.u(
        "<eCTwSCSequence><eCTwSC>1</eCTwSC><eCTwSC></eCTwSC></eCTwSCSequence>"
    )
...
class TestAttributeValueError(unittest.TestCase):
    ...
    def testAssignment(self):
        ...
        self.assertEqual(
            str(e), "Type tCardCymru enumeration constraint violated by value four"
        )
...
class TestExtraSimpleContentError(unittest.TestCase):
    ...
    def testException(self):
        ...
        self.assertTrue(e.location is None)
        self.assertEqual(
            str(e), "Instance of tCTwSC already has simple content value assigned"
        )
...
\tA wildcard per trac26.xsd[84:8]"""
    ...
    def testDocument2(self):
        ...
        self.assertEqual(acceptable[0].typeDefinition(), trac26.eInt.typeDefinition())
        self.assertEqual(
            str(e), "Invalid content something{} (expect eInt)".format(loctext)
        )
...
\tcardinal (1 instances)"""
    ...
    def testSchemaSupport(self):
        instance = trac26.eTranslateCard(
            trac26.eConcCardCymru("un"), trac26.eConcCardEnglish("one"), xs.int(1)
        )
        self.assertTrue(instance.validateBinding())
        instance = trac26.CreateFromDocument(self.Good_xmlt)
        self.assertEqual(self.Good_xmld, instance.toxml("utf-8", root_only=True))
...
\tcardinal (1 instances)"""
    ...
    def testSchemaSupport(self):
        instance = trac26.eTranslateCardMulti(
            trac26.eConcCardCymru("un"), trac26.eConcCardEnglish("one"), xs.int(1)
        )
        self.assertTrue(instance.validateBinding())
...
class TestContentInNilInstanceError(unittest.TestCase):
    Good_xmlt = six.u(
        '<eNilCTwSC xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:nil="true"/>'
    )
    Good_xmld = Good_xmlt.encode("utf-8")
    Bad_xmlt = six.u(
        '<eNilCTwSC xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:nil="true">4</eNilCTwSC>'
    )
    Bad_xmld = Bad_xmlt.encode("utf-8")
...
class TestSimpleUnionValueError(unittest.TestCase):
    ...
    def testSchemaSupport(self):
        for ty, va in (
            (trac26.tCardCymru, "un"),
            (trac26.tCardEnglish, "two"),
            (xs.int, 3),
        ):
