...
class TestTrac_0139(unittest.TestCase):
    ...
    def testASCII_libxml2_str(self):
        if not self.have_libxml2:
            _log.warning(
                "%s: testASCII_libxml2_str bypassed since libxml2 not present", __file__
            )
            return
        self.useLibXML2Parser()
        instance = CreateFromDocument(self.ascii_xmld)
    ...
    def testASCII_libxml2_file(self):
        if not self.have_libxml2:
            _log.warning(
                "%s: testASCII_libxml2_file bypassed since libxml2 not present",
                __file__,
            )
            return
        self.useLibXML2Parser()
    ...
    def testNihongo_expat_text(self):
        self.assertRaises(
            xml.sax.SAXParseException, CreateFromDocument, self.nihongo_xmlt
        )

    def testNihongo_expat_data(self):
        self.assertRaises(
            xml.sax.SAXParseException, CreateFromDocument, self.nihongo_xmld
        )
    ...
    def testNihongo_libxml2_str(self):
        if not self.have_libxml2:
            _log.warning(
                "%s: testNihongo_libxml2_str bypassed since libxml2 not present",
                __file__,
            )
            return
        self.assertRaises(
    ...
    def testNihongo_libxml2_file(self):
        if not self.have_libxml2:
            _log.warning(
                "%s: testNihongo_libxml2_file bypassed since libxml2 not present",
                __file__,
            )
            return
        self.useLibXML2Parser()
