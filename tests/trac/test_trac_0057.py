...
class TestTrac_0057(unittest.TestCase):
    ...
    def testDefault(self):
        ...
        if sys.version_info[:2] >= (2, 7):
            ...
            self.assertTrue(
                isinstance(cm.exception.instance, ObsProject.typeDefinition())
            )
            # Verify the exception tells us what was rejected
            time_of_creation_ed = ObsProject.typeDefinition()._UseForTag(
                Namespace.createExpandedName("timeOfCreation")
            )
            self.assertTrue(
                isinstance(
                    cm.exception.value,
                    time_of_creation_ed.elementBinding().typeDefinition(),
                )
            )
            ...
            assigned_priority_ed = ObsProject.typeDefinition()._UseForTag(
                Namespace.createExpandedName("assignedPriority")
            )
    ...
    def testDisable(self):
        ...
        self.assertRaises(
            pyxb.IncompleteElementContentError, self.exec_toxmld, instance
        )
        if sys.version_info[:2] >= (2, 7):
            ...
            self.assertEqual(
                instance.timeOfCreation, cm.exception.symbol_set[time_of_creation_ed][0]
            )
