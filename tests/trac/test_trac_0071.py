...
class TestTrac_0071(unittest.TestCase):
    def testFieldConstructor(self):
        field = field_type(
            "title", pyxb.BIND("foo", lang="ENG"), _element=field_element
        )
    ...
    def testMetaConstructor(self):
        ...
        newdoc.timespan.append(
            pyxb.BIND(  # Single timespan
                pyxb.BIND(  # First field instance
                    "title", pyxb.BIND("foo", lang="ENG")
                ),
                start="-INF",
                end="+INF",
            )
    ...
    def testMetaBadFieldName(self):
        ...
        if sys.version_info[:2] < (2, 7):
            self.assertRaises(
                pyxb.UnrecognizedContentError, newdoc.timespan.append, ts_bind
            )
            return
    ...
    def testMetaGoodBind(self):
        ...
        bind = pyxb.BIND(  # First field instance
            name="title", value_=[pyxb.BIND("foo", lang="ENG")]
        )
