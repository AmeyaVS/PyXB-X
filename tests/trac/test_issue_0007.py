...
class TestIssue0007(unittest.TestCase):
    def testConstruction(self):
        ...
        i = number(53)
        self.assertEqual(
            six.u("<number>53</number>").encode("utf-8"),
            i.toxml("utf-8", root_only=True),
        )
        self.assertTrue(i.bounded)
        # Explicit assignment of attribute makes it explicit even if same as default
        i.bounded = True
        self.assertEqual(
            six.u('<number bounded="true">53</number>').encode("utf-8"),
            i.toxml("utf-8", root_only=True),
        )
        i.bounded = False
        self.assertEqual(
            six.u('<number bounded="false">53</number>').encode("utf-8"),
            i.toxml("utf-8", root_only=True),
        )
    ...
    def testNilConstruction(self):
        ...
        if sys.version_info[1] < 8:
            ...
            self.assertEqual(
                six.u(
                    '<number xsi:nil="true" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"/>'
                ).encode("utf-8"),
                i.toxml("utf-8", root_only=True),
            )
    ...
    def testReplacement(self):
        ...
        i._setValue(27)
        self.assertEqual(
            six.u('<number bounded="true">27</number>').encode("utf-8"),
            i.toxml("utf-8", root_only=True),
        )
