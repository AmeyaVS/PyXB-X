...
uri = (
    "http://www.weather.gov/forecasts/xml/sample_products/browser_interface/ndfdXMLclient.php?zipCodeList=%s&product=time-series&begin=%s&end=%s&maxt=maxt&mint=mint"
...
for i in range(len(data.location)):
    ...
    for p in data.parameters:
        ...
        for t in p.temperature:
            ...
            print(
                "%s (%s): %s"
                % (t.name[0], t.units, " ".join([str(_v) for _v in t.content()]))
            )
        ...
        for ti in range(
            min(
                len(mint_time_layout.start_valid_time),
                len(maxt_time_layout.start_valid_time),
            )
        ):
