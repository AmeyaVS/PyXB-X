...
class decimal(
    ...
    basis._NoNullaryNonNillableNew_mixin,
):
...
class _fp(
    basis.simpleTypeDefinition, six.float_type, basis._NoNullaryNonNillableNew_mixin
):
    _XsdBaseType = anySimpleType
...
class duration(
    basis.simpleTypeDefinition, datetime.timedelta, basis._RepresentAsXsdLiteral_mixin
):
    ...
    __Lexical_re = re.compile(
        r"^(?P<neg>-?)P((?P<years>\d+)Y)?((?P<months>\d+)M)?((?P<days>\d+)D)?(?P<Time>T((?P<hours>\d+)H)?((?P<minutes>\d+)M)?(((?P<seconds>\d+)(?P<fracsec>\.\d+)?)S)?)?$"
    )
    ...
    def __new__(cls, *args, **kw):
        ...
        if not have_kw_update:
            ...
            if 0 != (rem_time % 1):
                data["microseconds"] = data.pop("microseconds", 0) + six.int_type(
                    1000000 * (rem_time % 1)
                )
                rem_time = rem_time // 1
...
class _PyXBDateTime_base(
    basis.simpleTypeDefinition, basis._RepresentAsXsdLiteral_mixin
):
    ...
    def _LexicalToKeywords(cls, text):
        ...
        if match_map.get("fracsec") is not None:
            kw["microsecond"] = six.int_type(
                round(1000000 * six.float_type("0%s" % (match_map["fracsec"],)))
            )
        else:
            # Discard any bogosity passed in by the caller
            kw.pop("microsecond", None)
    ...
    def __new__(cls, *args, **kw):
        ...
        if kw.get("_nil"):
            ...
            if isinstance(value, six.string_types):
                ...
                raise TypeError(
                    "function takes at least 3 arguments (%d given)" % (len(args),)
                )
            else:
                raise SimpleTypeValueError(cls, value)
        elif 3 <= len(args):
    ...
    def __new__(cls, *args, **kw):
        ...
        if kw.get("_nil"):
            ...
            if isinstance(value, six.string_types):
                if 1 != len(args):
                    raise TypeError(
                        "construction from string requires exactly 1 argument"
                    )
                ctor_kw.update(cls._LexicalToKeywords(value))
            elif isinstance(value, (datetime.date, datetime.datetime)):
                if 1 != len(args):
                    raise TypeError(
                        "construction from instance requires exactly 1 argument"
                    )
                ...
                if fi != len(args):
                    raise TypeError(
                        "function takes %d arguments plus optional tzinfo (%d given)"
                        % (len(cls._ValidFields), len(args))
                    )
        else:
            raise TypeError(
                "function takes %d arguments plus optional tzinfo"
                % (len(cls._ValidFields),)
            )
...
class QName(basis.simpleTypeDefinition, pyxb.namespace.ExpandedName):
    ...
    def _ConvertIf(cls, value, xmlns_context):
        ...
        if 0 <= value.find(":"):
            ...
            if (NCName._ValidRE.match(prefix) is None) or (
                NCName._ValidRE.match(local) is None
            ):
                raise SimpleTypeValueError(cls, value)
            if xmlns_context is None:
                raise pyxb.QNameResolutionError(
                    "QName resolution requires namespace context", value, xmlns_context
                )
    ...
    def _ConvertArguments_vx(cls, args, kw):
        if 1 == len(args):
            xmlns_context = kw.pop(
                "_xmlns_context", pyxb.namespace.NamespaceContext.Current()
            )
            args = (cls._ConvertIf(args[0], xmlns_context),)
        super_fn = getattr(
            super(QName, cls), "_ConvertArguments_vx", lambda *a, **kw: args
        )
    ...
    def _XsdConstraintsPreCheck_vb(cls, value):
        super_fn = getattr(
            super(QName, cls), "_XsdConstraintsPreCheck_vb", lambda *a, **kw: True
        )
        return super_fn(
            cls._ConvertIf(value, pyxb.namespace.NamespaceContext.Current())
        )
...
class normalizedString(string):
    ...
    def _ValidateString_va(cls, value):
        ...
        tests."""
        super_fn = getattr(
            super(normalizedString, cls), "_ValidateString_va", lambda *a, **kw: True
        )
...
class token(normalizedString):
    ...
    def _ValidateString_va(cls, value):
        super_fn = getattr(
            super(token, cls), "_ValidateString_va", lambda *a, **kw: True
        )
        if not super_fn(value):
            return False
        if value.startswith(" ") or value.endswith(" ") or (0 <= value.find("  ")):
            raise SimpleTypeValueError(cls, value)
        return True
...
class integer(
    basis.simpleTypeDefinition, six.long_type, basis._NoNullaryNonNillableNew_mixin
):
...
class anyType(basis.complexTypeDefinition):
    ...
    _DefinitionLocation = pyxb.utils.utility.Location(
        "http://www.w3.org/TR/2001/REC-xmlschema-1-20010502/#key-urType", 1, 1
    )
    _ContentTypeTag = basis.complexTypeDefinition._CT_MIXED
    _Abstract = False
    _HasWildcardElement = True
    _AttributeWildcard = content.Wildcard(
        namespace_constraint=content.Wildcard.NC_any,
...
def _BuildAutomaton():
    ...
    states.add(st_0)
    transitions = set()
    transitions.add(fac.Transition(st_0, [fac.UpdateInstruction(cc_0, True)]))
