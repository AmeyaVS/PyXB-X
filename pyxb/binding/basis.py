...
class _TypeBinding_mixin(utility.Locatable_mixin):
    ...
    def _RequireXSIType(cls, value_type):
        ...
        if issubclass(cls, STD_union):
            ...
            raise pyxb.LogicError(
                "Union %s instance type %s not sublass of member type?"
                % (cls, value_type)
            )
    ...
    def _CompatibleValue(cls, value, **kw):
        ...
        if (pyxb.binding.datatypes.anySimpleType == cls) and issubclass(
            value_type, simpleTypeDefinition
        ):
            return value
        if pyxb.binding.datatypes.anyType == cls:
            if not isinstance(value, _TypeBinding_mixin):
                _log.info(
                    "Created %s instance from value of type %s",
                    cls._ExpandedName,
                    type(value),
                )
                value = cls(value)
    ...
    def _IsSimpleTypeContent(cls):
        ...
        raise pyxb.LogicError(
            "Failed to override _TypeBinding_mixin._IsSimpleTypeContent"
        )
    ...
    def toDOM(self, bds=None, parent=None, element_name=None):
        ...
        if isinstance(element_name, six.string_types):
            element_name = pyxb.namespace.ExpandedName(
                bds.defaultNamespace(), element_name
            )
        if (element_name is None) and (self._element() is not None):
            ...
            need_xsi_type = (
                need_xsi_type
                or element_binding.typeDefinition()._RequireXSIType(type(self))
            )
    ...
    def _toDOM_csc(self, dom_support, parent):
        ...
        return getattr(
            super(_TypeBinding_mixin, self),
            "_toDOM_csc",
            lambda *_args, **_kw: dom_support,
        )(dom_support, parent)
...
class simpleTypeDefinition(
    _TypeBinding_mixin, utility._DeconflictSymbols_mixin, _DynamicCreate_mixin
):
    ...
    _ReservedSymbols = _TypeBinding_mixin._ReservedSymbols.union(
        set(
            [
                "XsdLiteral",
                "xsdLiteral",
    ...
    def _InitializeFacetMap(cls, *args):
        ...
        if fm is not None:
            raise pyxb.LogicError(
                "%s facet map initialized multiple times: %s"
                % (cls.__name__, cls.__FacetMapAttributeName())
            )
        ...
        while fm is None:
            ...
            for super_class in source_class.mro():
                ...
                if (
                    super_class == simpleTypeDefinition
                ):  # and (source_class.XsdSuperType() is not None):
                    break
        ...
        if fm is None:
            raise pyxb.LogicError(
                "%s is not a child of simpleTypeDefinition" % (cls.__name__,)
            )
        fm = fm.copy()
        for facet in args:
            fm[type(facet)] = facet
    ...
    def __init__(self, *args, **kw):
        ...
        """
        # PyXBFactoryKeywords
        validate_constraints = kw.pop(
            "_validate_constraints", self._validationConfig.forBinding
        )
    ...
    def _SimpleTypeDefinition(cls, std):
        ...
        if hasattr(cls, attr_name):
            ...
            if old_value != std:
                raise pyxb.LogicError(
                    "%s: Attempt to override existing STD %s with %s"
                    % (cls, old_value.name(), std.name())
                )
    ...
    def XsdSuperType(cls):
        ...
        for sc in cls.mro():
            ...
            if (not sc.__name__.startswith("_")) and issubclass(
                sc, simpleTypeDefinition
            ):
                return sc
    ...
    def _XsdConstraintsPreCheck_vb(cls, value):
        ...
        """
        super_fn = getattr(
            super(simpleTypeDefinition, cls),
    ...
    def XsdValueLength(cls, value):
        ...
        if not hasattr(cls, "_XsdValueLength_vx"):
            raise pyxb.LogicError(
                "Class %s does not support length validation" % (cls.__name__,)
            )
    ...
    def _description(cls, name_only=False, user_documentation=True):
        ...
        desc = [
            name,
            " restriction of ",
            cls.XsdSuperType()._description(name_only=True),
        ]
...
class STD_union(simpleTypeDefinition):
    ...
    def Factory(cls, *args, **kw):
        ...
        validate_constraints = kw.get(
            "_validate_constraints", cls._GetValidationConfig().forBinding
        )
    ...
    def __new__(self, *args, **kw):
        raise pyxb.LogicError(
            "%s: cannot construct instances of union" % (self.__class__.__name__,)
        )
    ...
    def _description(cls, name_only=False, user_documentation=True):
        ...
        desc = [name, ", union of "]
        desc.append(
            ", ".join([_td._description(name_only=True) for _td in cls._MemberTypes])
        )
        return "".join(desc)
...
class STD_list(simpleTypeDefinition, six.list_type):
    ...
    def _ConvertArguments_vx(cls, args, kw):
        ...
        super_fn = getattr(
            super(STD_list, cls), "_ConvertArguments_vx", lambda *a, **kw: args
        )
...
class element(utility._DeconflictSymbols_mixin, _DynamicCreate_mixin):
    ...
    def _real_substitutesFor(self, other):
        ...
        if self.name().elementBinding() == other:
            return True
        return (
            self.substitutionGroup() == other
        ) or self.substitutionGroup().substitutesFor(other)
    ...
    def __init__(
        ...
        if unicode_default is not None:
            ...
            self.__defaultValue = self.__typeDefinition.Factory(
                unicode_default, _from_xml=True
            )
    ...
    def __call__(self, *args, **kw):
        ...
        assert (
            dom_node is None
        ), "Cannot pass DOM node directly to element constructor; use createFromDOM"
        if "_element" in kw:
            raise pyxb.LogicError(
                "Cannot set _element in element-based instance creation"
            )
        kw["_element"] = self
    ...
    def compatibleValue(self, value, **kw):
        ...
        if (
            isinstance(value, _TypeBinding_mixin)
            and (value._element() is not None)
            and value._element().substitutesFor(self)
        ):
            return value
        if self.abstract():
            location = None
    ...
    def CreateDOMBinding(cls, node, element_binding, **kw):
        ...
        (did_replace, type_class) = XSI._InterpretTypeAttribute(
            XSI.type.getAttribute(node), ns_ctx, fallback_namespace, type_class
        )
    ...
    def AnyCreateFromDOM(cls, node, fallback_namespace):
        ...
        expanded_name = pyxb.namespace.ExpandedName(
            node, fallback_namespace=fallback_namespace
        )
        return cls.CreateDOMBinding(
            node, expanded_name.elementBinding(), _fallback_namespace=fallback_namespace
        )
    ...
    def elementForName(self, name):
        ...
        assert (
            "elementBinding" in elt_en.namespace()._categoryMap()
        ), "No element bindings in %s" % (elt_en.namespace(),)
    ...
    def _description(self, name_only=False, user_documentation=True):
        ...
        if self.substitutionGroup() is not None:
            desc.extend(
                [
                    ", substitutes for ",
                    self.substitutionGroup()._description(name_only=True),
                ]
            )
...
class ElementContent(_Content):
    ...
    def __init__(self, value, element_declaration=None, instance=None, tag=None):
        ...
        assert (element_declaration is None) or isinstance(
            element_declaration, pyxb.binding.content.ElementDeclaration
        )
...
class complexTypeDefinition(
    ...
    def __init__(self, *args, **kw):
        ...
        if dom_node is not None:
            if (location is None) and isinstance(
                dom_node, pyxb.utils.utility.Locatable_mixin
            ):
                location = dom_node._location()
        ...
        if 0 < len(args):
            if did_set_kw_elt:
                raise pyxb.UsageError(
                    "Cannot mix keyword and positional args for element initialization"
                )
            self.extend(args, _from_xml=from_xml, _location=location)
        elif self._CT_SIMPLE == self._ContentTypeTag:
            value = self._TypeDefinition.Factory(
                _require_value=not self._isNil(),
                _dom_node=dom_node,
                _location=location,
                _nil=self._isNil(),
                _apply_attributes=False,
                *args
            )
    ...
    def _validateBinding_vx(self):
        if self._isNil():
            if (
                self._IsSimpleTypeContent() and (self.__content is not None)
            ) or self.__content:
        ...
        for content in order:
            ...
            if isinstance(content.value, _TypeBinding_mixin):
                ...
                _log.warning(
                    "Cannot validate value %s in field %s",
                    content.value,
                    content.elementDeclaration.id(),
                )
    ...
    def _resetAutomaton(self):
        if self._Automaton is not None:
            if self.__automatonConfiguration is None:
                ...
                self.__automatonConfiguration = (
                    pyxb.binding.content.AutomatonConfiguration(self)
                )
    ...
    def append(self, value, **kw):
        ...
        require_validation = kw.get(
            "_require_validation", self._validationConfig.forBinding
        )
        ...
        if isinstance(value, xml.dom.Node):
            ...
            if node.nodeType in (
                ...
                (element_binding, element_decl) = self._ElementBindingDeclForName(
                    expanded_name
                )
                if element_binding is not None:
                    # If we have an element binding, we need to use it because
                    # it knows how to resolve substitution groups.
                    value = element_binding._createFromDOM(
                        node, expanded_name, _fallback_namespace=fallback_namespace
                    )
                else:
                    ...
                    if xsi_type is not None:
                        ns_ctx = pyxb.namespace.NamespaceContext.GetNodeContext(node)
                        (try_create, type_class) = XSI._InterpretTypeAttribute(
                            xsi_type, ns_ctx, fallback_namespace, None
                        )
                    if try_create:
                        value = element.CreateDOMBinding(
                            node, None, _fallback_namespace=fallback_namespace
                        )
                    else:
                        _log.warning(
                            "Unable to convert DOM node %s at %s to binding",
                            expanded_name,
                            getattr(node, "location", "[UNAVAILABLE]"),
                        )
        if (
            (not maybe_element)
            and isinstance(value, six.string_types)
            and (self._ContentTypeTag in (self._CT_EMPTY, self._CT_ELEMENT_ONLY))
        ):
        ...
        # element content.
        if (
            (element_binding is not None)
            or isinstance(value, (xml.dom.Node, complexTypeDefinition, pyxb.BIND))
            or (
                isinstance(value, simpleTypeDefinition)
                and not (self._IsSimpleTypeContent() or self._IsMixed())
            )
        ):
            ...
            if self.__automatonConfiguration:
                raise pyxb.UnrecognizedContentError(
                    self, self.__automatonConfiguration, value, location
                )
    ...
    def _appendWildcardElement(self, value):
        if isinstance(value, xml.dom.Node) or (
            isinstance(value, _TypeBinding_mixin) and (value._element is not None)
        ):
    ...
    def extend(
        self, value_list, _fallback_namespace=None, _from_xml=False, _location=None
    ):
        """Invoke L{append} for each value in the list, in turn."""
        kw = {
            "_fallback_namespace": _fallback_namespace,
            "_from_xml": _from_xml,
            "_location": _location,
        }
    ...
    def _toDOM_csc(self, dom_support, parent):
        ...
        if self._isNil():
            ...
            for mc in mixed_content:
                pass
        return getattr(
            super(complexTypeDefinition, self),
            "_toDOM_csc",
            lambda *_args, **_kw: dom_support,
        )(dom_support, parent)
    ...
    def _description(cls, name_only=False, user_documentation=True):
        ...
        if cls._CT_EMPTY == cls._ContentTypeTag:
            ...
            desc.extend(
                [
                    ", simple content type ",
                    cls._TypeDefinition._description(name_only=True),
                ]
            )
        else:
        ...
        if (0 < len(cls._AttributeMap)) or (cls._AttributeWildcard is not None):
            ...
            desc.append(
                "\n  ".join(
                    [
                        _au._description(user_documentation=False)
                        for _au in six.itervalues(cls._AttributeMap)
                    ]
                )
            )
        ...
        if (0 < len(cls._ElementMap)) or cls._HasWildcardElement:
            ...
            desc.append(
                "\n  ".join(
                    [
                        _eu._description(user_documentation=False)
                        for _eu in six.itervalues(cls._ElementMap)
                    ]
                )
            )
