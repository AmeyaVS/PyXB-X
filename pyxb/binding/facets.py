...
class Facet(pyxb.cscRoot):
    ...
    def _setFromKeywords_vb(self, **kw):
        ...
        # values.
        assert (self.__valueDatatype is not None) or (
            self.__baseTypeDefinition is not None
        )
        super_fn = getattr(
            super(Facet, self), "_setFromKeywords_vb", lambda *a, **kw: self
        )
        return super_fn(**kw)
...
class ConstrainingFacet(Facet):
    ...
    def _validateConstraint_vx(self, value):
        raise pyxb.LogicError(
            "Facet %s does not implement constraints" % (self.Name(),)
        )
    ...
    def _setFromKeywords_vb(self, **kw):
        ...
        * value
        ...
        super_fn = getattr(
            super(ConstrainingFacet, self), "_setFromKeywords_vb", lambda *a, **kw: self
        )
        rv = super_fn(**kw)
...
class _LateDatatype_mixin(pyxb.cscRoot):
    ...
    def bindValueDatatype(self, value_datatype):
        self.setFromKeywords(
            _constructor=True, value_datatype=self.BindingValueDatatype(value_datatype)
        )
...
class _Fixed_mixin(pyxb.cscRoot):
    ...
    def _setFromKeywords_vb(self, **kw):
        ...
        * fixed
        ...
        self.__setFromKeywords(**kw)
        super_fn = getattr(
            super(_Fixed_mixin, self), "_setFromKeywords_vb", lambda *a, **kw: self
        )
        return super_fn(**kw)
...
class _CollectionFacet_mixin(pyxb.cscRoot):
    ...
    def _setFromKeywords_vb(self, **kw):
        ...
        if not kw.get("_constructor", False):
            self.__items.append(
                self._CollectionFacet_itemType(facet_instance=self, **kw)
            )
        super_fn = getattr(
            super(_CollectionFacet_mixin, self),
            "_setFromKeywords_vb",
            lambda *a, **kw: self,
        )
        return super_fn(**kw)
...
class CF_length(ConstrainingFacet, _Fixed_mixin):
    ...
    def _validateConstraint_vx(self, value):
        ...
        return (
            (value_length is None)
            or (self.value() is None)
            or (value_length == self.value())
        )
...
class CF_pattern(ConstrainingFacet, _CollectionFacet_mixin):
    ...
    def patternElements(self):
        return self.__patternElements
...
class _EnumerationElement(object):
    ...
    def __init__(
        ...
        **kw
        ...
        self.__value = value_datatype.Factory(
            value, _validate_constraints=False, _from_xml=True
        )
...
_WhiteSpace_enum.preserve = _WhiteSpace_enum._CF_enumeration.addEnumeration(
    unicode_value=six.u("preserve"), tag="preserve"
)
_WhiteSpace_enum.replace = _WhiteSpace_enum._CF_enumeration.addEnumeration(
    unicode_value=six.u("replace"), tag="replace"
)
_WhiteSpace_enum.collapse = _WhiteSpace_enum._CF_enumeration.addEnumeration(
    unicode_value=six.u("collapse"), tag="collapse"
)
...
class CF_whiteSpace(ConstrainingFacet, _Fixed_mixin):
    ...
    def normalizeString(self, value):
        ...
        assert (
            self.value() == _WhiteSpace_enum.collapse
        ), 'Unexpected value "%s" for whiteSpace facet' % (self.value(),)
...
class FundamentalFacet(Facet):
    ...
    def CreateFromDOM(cls, node, owner_type_definition, base_type_definition=None):
        ...
        rv = facet_class(
            base_type_definition=base_type_definition,
...
FundamentalFacet.Facets = [FF_equal, FF_ordered, FF_bounded, FF_cardinality, FF_numeric]
...
datatypes.ENTITIES._InitializeFacetMap(
    datatypes.ENTITIES._CF_pattern,
...
datatypes.IDREFS._InitializeFacetMap(
    datatypes.IDREFS._CF_pattern,
...
datatypes.NMTOKENS._InitializeFacetMap(
    datatypes.NMTOKENS._CF_pattern,
...
datatypes.NOTATION._InitializeFacetMap(
    datatypes.NOTATION._CF_minLength,
...
datatypes.QName._InitializeFacetMap(
...
datatypes.anyURI._InitializeFacetMap(
    datatypes.anyURI._CF_minLength,
...
datatypes.base64Binary._CF_enumeration = CF_enumeration(
    value_datatype=datatypes.base64Binary
)
...
datatypes.base64Binary._InitializeFacetMap(
    datatypes.base64Binary._CF_minLength,
...
datatypes.boolean._InitializeFacetMap(
    datatypes.boolean._CF_whiteSpace, datatypes.boolean._CF_pattern
)
datatypes.byte._CF_minInclusive = CF_minInclusive(
    value_datatype=datatypes.byte, value=datatypes.anySimpleType(six.u("-128"))
)
datatypes.byte._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.byte, value=datatypes.anySimpleType(six.u("127"))
)
datatypes.byte._InitializeFacetMap(
    datatypes.byte._CF_minInclusive, datatypes.byte._CF_maxInclusive
)
datatypes.date._CF_pattern = CF_pattern()
datatypes.date._CF_minInclusive = CF_minInclusive(value_datatype=datatypes.date)
datatypes.date._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.date._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.date._InitializeFacetMap(
...
datatypes.dateTime._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.dateTime._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.dateTime._InitializeFacetMap(
    datatypes.dateTime._CF_pattern,
...
datatypes.decimal._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.decimal._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.decimal._InitializeFacetMap(
    datatypes.decimal._CF_totalDigits,
...
datatypes.double._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.double._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.double._InitializeFacetMap(
    datatypes.double._CF_pattern,
...
datatypes.duration._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.duration._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.duration._InitializeFacetMap(
    datatypes.duration._CF_pattern,
...
datatypes.float._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.float._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.float._InitializeFacetMap(
    datatypes.float._CF_pattern,
...
datatypes.gDay._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.gDay._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.gDay._InitializeFacetMap(
    datatypes.gDay._CF_pattern,
...
datatypes.gMonth._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.gMonth._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.gMonth._InitializeFacetMap(
    datatypes.gMonth._CF_pattern,
...
datatypes.gMonthDay._CF_minInclusive = CF_minInclusive(
    value_datatype=datatypes.gMonthDay
)
datatypes.gMonthDay._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.gMonthDay._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.gMonthDay._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.gMonthDay
)
datatypes.gMonthDay._InitializeFacetMap(
    datatypes.gMonthDay._CF_pattern,
...
datatypes.gYear._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.gYear._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.gYear._InitializeFacetMap(
    datatypes.gYear._CF_pattern,
...
datatypes.gYearMonth._CF_minInclusive = CF_minInclusive(
    value_datatype=datatypes.gYearMonth
)
datatypes.gYearMonth._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.gYearMonth._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.gYearMonth._CF_enumeration = CF_enumeration(
    value_datatype=datatypes.gYearMonth
)
datatypes.gYearMonth._CF_whiteSpace = CF_whiteSpace(value=_WhiteSpace_enum.collapse)
datatypes.gYearMonth._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.gYearMonth
)
datatypes.gYearMonth._InitializeFacetMap(
    datatypes.gYearMonth._CF_pattern,
...
datatypes.hexBinary._InitializeFacetMap(
    datatypes.hexBinary._CF_minLength,
...
datatypes.int._CF_minInclusive = CF_minInclusive(
    value_datatype=datatypes.int, value=datatypes.anySimpleType(six.u("-2147483648"))
)
datatypes.int._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.int, value=datatypes.anySimpleType(six.u("2147483647"))
)
datatypes.int._InitializeFacetMap(
    datatypes.int._CF_minInclusive, datatypes.int._CF_maxInclusive
)
datatypes.integer._CF_pattern = CF_pattern()
datatypes.integer._CF_pattern.addPattern(pattern=six.u("[\\-+]?[0-9]+"))
datatypes.integer._CF_fractionDigits = CF_fractionDigits(
    value=datatypes.nonNegativeInteger(0)
)
datatypes.integer._InitializeFacetMap(
    datatypes.integer._CF_pattern, datatypes.integer._CF_fractionDigits
)
datatypes.language._CF_pattern = CF_pattern()
datatypes.language._CF_pattern.addPattern(
    pattern=six.u("[a-zA-Z]{1,8}(-[a-zA-Z0-9]{1,8})*")
)
datatypes.language._InitializeFacetMap(datatypes.language._CF_pattern)
datatypes.long._CF_minInclusive = CF_minInclusive(
    value_datatype=datatypes.long,
    value=datatypes.anySimpleType(six.u("-9223372036854775808")),
)
datatypes.long._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.long,
    value=datatypes.anySimpleType(six.u("9223372036854775807")),
)
datatypes.long._InitializeFacetMap(
    datatypes.long._CF_minInclusive, datatypes.long._CF_maxInclusive
)
datatypes.negativeInteger._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.negativeInteger, value=datatypes.anySimpleType(six.u("-1"))
)
datatypes.negativeInteger._InitializeFacetMap(
    datatypes.negativeInteger._CF_maxInclusive
)
datatypes.nonNegativeInteger._CF_minInclusive = CF_minInclusive(
    value_datatype=datatypes.nonNegativeInteger,
    value=datatypes.anySimpleType(six.u("0")),
)
datatypes.nonNegativeInteger._InitializeFacetMap(
    datatypes.nonNegativeInteger._CF_minInclusive
)
datatypes.nonPositiveInteger._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.nonPositiveInteger,
    value=datatypes.anySimpleType(six.u("0")),
)
datatypes.nonPositiveInteger._InitializeFacetMap(
    datatypes.nonPositiveInteger._CF_maxInclusive
)
datatypes.normalizedString._CF_whiteSpace = CF_whiteSpace(
    value=_WhiteSpace_enum.replace
)
datatypes.normalizedString._InitializeFacetMap(
    datatypes.normalizedString._CF_whiteSpace
)
datatypes.positiveInteger._CF_minInclusive = CF_minInclusive(
    value_datatype=datatypes.positiveInteger, value=datatypes.anySimpleType(six.u("1"))
)
datatypes.positiveInteger._InitializeFacetMap(
    datatypes.positiveInteger._CF_minInclusive
)
datatypes.short._CF_minInclusive = CF_minInclusive(
    value_datatype=datatypes.short, value=datatypes.anySimpleType(six.u("-32768"))
)
datatypes.short._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.short, value=datatypes.anySimpleType(six.u("32767"))
)
datatypes.short._InitializeFacetMap(
    datatypes.short._CF_minInclusive, datatypes.short._CF_maxInclusive
)
...
datatypes.string._InitializeFacetMap(
    datatypes.string._CF_minLength,
...
datatypes.time._CF_maxExclusive = CF_maxExclusive(
    value_datatype=datatypes.anySimpleType
)
datatypes.time._CF_minExclusive = CF_minExclusive(
    value_datatype=datatypes.anySimpleType
)
...
datatypes.time._InitializeFacetMap(
    datatypes.time._CF_pattern,
...
datatypes.unsignedByte._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.unsignedByte, value=datatypes.anySimpleType(six.u("255"))
)
datatypes.unsignedByte._InitializeFacetMap(datatypes.unsignedByte._CF_maxInclusive)
datatypes.unsignedInt._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.unsignedInt,
    value=datatypes.anySimpleType(six.u("4294967295")),
)
datatypes.unsignedInt._InitializeFacetMap(datatypes.unsignedInt._CF_maxInclusive)
datatypes.unsignedLong._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.unsignedLong,
    value=datatypes.anySimpleType(six.u("18446744073709551615")),
)
datatypes.unsignedLong._InitializeFacetMap(datatypes.unsignedLong._CF_maxInclusive)
datatypes.unsignedShort._CF_maxInclusive = CF_maxInclusive(
    value_datatype=datatypes.unsignedShort,
    value=datatypes.anySimpleType(six.u("65535")),
)
