...
DefaultArchivePrefix = os.path.realpath(
...
class NamespaceArchive(object):
        ...
        if reset:
            ...
            if archive_path is not None:
                ...
                ordered_archives = sorted(
                    list(archive_set), key=lambda _a: _a.archivePath()
                )
                ...
                for a in ordered_archives:
                    ...
                    if 0 < len(prereqs):
                        for p in prereqs:
                            ...
                            if da is None:
                                _log.warning(
                                    "%s depends on unavailable archive %s", a, p
                                )
    ...
    _STAGE_readModules = (
        2  # Read module records from archive, which includes UID dependences
    )
    _STAGE_validateModules = 3  # Verified pre-requisites for module loading
    _STAGE_readComponents = (
        4  # Extracted components from archive and integrated into namespaces
    )
    ...
    def __init__(
        self, archive_path=None, generation_uid=None, loadable=True, stage=None
    ):
        ...
        if generation_uid is not None:
            if archive_path:
                raise pyxb.LogicError(
                    "NamespaceArchive: cannot define both namespaces and archive_path"
                )
            ...
            if generation_uid is not None:
                raise pyxb.LogicError(
                    "NamespaceArchive: cannot provide generation_uid with archive_path"
                )
    ...
    def __createUnpickler(self):
        ...
        if self.__PickleFormat != fmt:
            raise pyxb.NamespaceArchiveError(
                "Archive format is %s, require %s" % (fmt, self.__PickleFormat)
            )
    ...
    def __readModules(self, unpickler):
        mrs = unpickler.load()
        assert isinstance(mrs, set), "Expected set got %s from %s" % (
            type(mrs),
            self.archivePath(),
        )
            ...
            for mr in mrs:
                ...
                if not (mr2 in self.__moduleRecords):
                    raise pyxb.NamespaceArchiveError(
                        ...
                        % (mr.namespace(), mr.generationUID(), self.archivePath())
                    )
        ...
        try:
            while self.__stage < stage:
                ...
                if self.__stage < self._STAGE_readComponents:
                    ...
                    continue
                raise pyxb.LogicError(
                    "Too many stages (at %s, want %s)" % (self.__stage, stage)
                )
        except:
            self.__stage = None
            self.__unpickler = None
...
class _ArchivableObject_mixin(pyxb.cscRoot):
    ...
    def _prepareForArchive(self, archive):
        ...
        if self._objectOrigin() is not None:
            return getattr(
                super(_ArchivableObject_mixin, self),
                "_prepareForArchive_csc",
                lambda *_args, **_kw: self,
            )(self._objectOrigin().moduleRecord())
...
class _NamespaceArchivable_mixin(pyxb.cscRoot):
    ...
    def _reset(self):
        ...
        getattr(
            super(_NamespaceArchivable_mixin, self), "_reset", lambda *args, **kw: None
        )()
    ...
    def _removeArchive(self, archive):
        ...
        assert not mr.isIncorporated(), "Removing archive %s after incorporation" % (
            archive.archivePath(),
        )
    ...
    def lookupModuleRecordByUID(
        self, generation_uid, create_if_missing=False, *args, **kw
    ):
    ...
    def _setState_csc(self, kw):
        ...
        return getattr(
            super(_NamespaceArchivable_mixin, self), "_getState_csc", lambda _kw: _kw
        )(kw)
    ...
    def markNotLoadable(self):
        ...
        if self._loadedFromArchive():
            raise pyxb.NamespaceError(
                self, "cannot mark not loadable when already loaded"
            )
        for mr in self.moduleRecords():
...
class ModuleRecord(pyxb.utils.utility.PrivateTransient_mixin):
    ...
    def setModulePath(self, module_path):
        if isinstance(module_path, six.string_types):
            self.__modulePath = ".".join(
                map(pyxb.utils.utility.MakeModuleElement, module_path.split("."))
            )
        else:
    ...
    def __init__(self, namespace, generation_uid, **kw):
        ...
        assert (
            generation_uid != builtin.BuiltInObjectUID
        ) or namespace.isBuiltinNamespace()
    ...
    def _setFromOther(self, other, archive):
        if (not self.__constructedLocally) or other.__constructedLocally:
            raise pyxb.ImplementationError(
                "Module record update requires local to be updated from archive"
            )
...
class _ObjectOrigin(pyxb.utils.utility.PrivateTransient_mixin, pyxb.cscRoot):
    ...
    def __init__(self, namespace, generation_uid, **kw):
        ...
        self.__moduleRecord = namespace.lookupModuleRecordByUID(
            generation_uid, create_if_missing=True, **kw
        )
...
class _SchemaOrigin(_ObjectOrigin):
    ...
    def __init__(self, **kw):
        ...
        super(_SchemaOrigin, self).__init__(
            kw.pop("namespace"), kw.pop("generation_uid"), **kw
        )
...
class NamespaceDependencies(object):
    ...
    def __init__(self, **kw):
        ...
        if 0 == len(namespace_set):
            raise pyxb.LogicError(
                "NamespaceDependencies requires at least one root namespace"
            )
