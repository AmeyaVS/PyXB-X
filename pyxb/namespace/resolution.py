...
class _Resolvable_mixin(pyxb.cscRoot):
    ...
    def isResolved(self):
        ...
        raise NotImplementedError(
            "_Resolvable_mixin.isResolved in %s" % (type(self).__name__,)
        )
    ...
    def _resolve(self):
        ...
        """
        raise NotImplementedError(
            "_Resolvable_mixin._resolve in %s" % (type(self).__name__,)
        )
...
class _NamespaceResolution_mixin(pyxb.cscRoot):
    ...
    def _reset(self):
        ...
        getattr(
            super(_NamespaceResolution_mixin, self), "_reset", lambda *args, **kw: None
        )()
    ...
    def _getState_csc(self, kw):
        ...
        return getattr(
            super(_NamespaceResolution_mixin, self), "_getState_csc", lambda _kw: _kw
        )(kw)
    ...
    def _setState_csc(self, kw):
        ...
        return getattr(
            super(_NamespaceResolution_mixin, self), "_setState_csc", lambda _kw: self
        )(kw)
    ...
    def queueForResolution(self, resolvable, depends_on=None):
        ...
        if not resolvable.isResolved():
            ...
            if depends_on is not None and not depends_on.isResolved():
                ...
                self.__unresolvedDependents.setdefault(resolvable, set()).add(
                    depends_on
                )
        return resolvable
    ...
    def _replaceComponent_csc(self, existing_def, replacement_def):
        ...
        try:
            ...
            if (replacement_def is None) or (
                replacement_def in self.__unresolvedComponents
            ):
            ...
            pass
        return getattr(
            super(_NamespaceResolution_mixin, self),
            "_replaceComponent_csc",
            lambda *args, **kw: replacement_def,
        )(existing_def, replacement_def)
    ...
    def resolveDefinitions(self, allow_unresolved=False):
        ...
        while 0 < len(self.__unresolvedComponents):
            ...
            for resolvable in unresolved:
                ...
                assert resolvable.isResolved() or (
                    resolvable in self.__unresolvedComponents
                ), "Lost resolvable %s" % (resolvable,)
                ...
                for d in self.__unresolvedComponents:
                    if isinstance(d, structures._NamedComponent_mixin):
                        failed_components.append(
                            "%s named %s" % (d.__class__.__name__, d.name())
                        )
                    else:
                        failed_components.append(
                            "Anonymous %s" % (d.__class__.__name__,)
                        )
                raise pyxb.NotInNamespaceError(
    ...
    def __keyForCompare(dependency_map):
        ...
        class K(object):
            ...
            def __lt__(self, other):
                return (self.__ns in dependency_map.get(other.__ns, set())) and not (
                    other.__ns in dependency_map.get(self.__ns, set())
                )
...
class NamespaceContext(object):
    ...
    def __str__(self):
        ...
        if self.defaultNamespace() is not None:
            rv.extend(
                ["(defaultNamespace=", six.text_type(self.defaultNamespace()), ") "]
            )
        if self.targetNamespace() is not None:
            rv.extend(
                ["(targetNamespace=", six.text_type(self.targetNamespace()), ") "]
            )
        rv.append("\n")
    ...
    def setDefaultNamespace(self, default_namespace):
        ...
        if isinstance(default_namespace, six.string_types):
            default_namespace = utility.NamespaceForURI(
    ...
    def declareNamespace(self, namespace, prefix=None, add_to_map=False):
        ...
        if ns:
            if ns != namespace:
                raise pyxb.LogicError(
                    "Prefix %s is already in use for %s" % (prefix, ns)
                )
            return prefix
        ...
        if uri:
            ...
            if prefix is not None:
                raise pyxb.NamespaceError(
                    self, "Attempt to undefine non-default namespace %s" % (prefix,)
                )
            self.__removePrefixMap(prefix)
    ...
    def finalizeTargetNamespace(self, tns_uri=None, including_context=None):
        if tns_uri is not None:
            ...
            self.__targetNamespace = utility.NamespaceForURI(
                tns_uri, create_if_missing=True
            )
        ...
        if self.__pendingReferencedNamespaces is not None:
            [
                self.__targetNamespace._referenceNamespace(_ns)
                for _ns in self.__pendingReferencedNamespaces
            ]
            self.__pendingReferencedNamespace = None
        assert self.__targetNamespace is not None
        if (
            not self.__fallbackToTargetNamespace
        ) and self.__targetNamespace.isAbsentNamespace():
            self.__fallbackToTargetNamespace = True
    ...
    def __init__(
        ...
        if parent_context is not None:
            ...
            self.__fallbackToTargetNamespace = (
                parent_context.__fallbackToTargetNamespace
            )
            ...
            for ai in range(dom_node.attributes.length):
                ...
                if builtin.XMLNamespaces.uri() == attr.namespaceURI:
                    ...
                    if attr.namespaceURI is not None:
                        uri = utility.NamespaceForURI(
                            attr.namespaceURI, create_if_missing=True
                        )
        ...
        if 0 <= name.find(":"):
            ...
            if namespace is None:
                raise pyxb.QNameResolutionError(
                    "No namespace declaration for prefix", name, self
                )
        else:
            local_name = name
            ...
            if (namespace is None) and not default_no_namespace:
                raise pyxb.QNameResolutionError(
                    "NCName with no fallback/default namespace cannot be resolved",
                    name,
                    self,
                )
