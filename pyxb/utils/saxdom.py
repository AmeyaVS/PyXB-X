...
def _DumpDOM(n, depth=0):
    ...
    if xml.dom.Node.ELEMENT_NODE == n.nodeType:
        print(
            "%sElement[%d] %s %s with %d children"
            % (
                pfx,
                n._indexInParent(),
                n,
                pyxb.namespace.ExpandedName(n.name),
                len(n.childNodes),
            )
        )
        ...
        print(
            "%s%s"
            % (pfx, " ; ".join(["%s=%s" % (_k, _v.uri()) for (_k, _v) in ins.items()]))
        )
...
class _DOMSAXHandler(pyxb.utils.saxutils.BaseSAXHandler):
    ...
    def startElementNS(self, name, qname, attrs):
        ...
        for name in attrs.getNames():
            ...
            this_state.__attributes._addItem(
                Attr(
                    expanded_name=attr_en,
                    namespace_context=ns_ctx,
                    value=value,
                    location=this_state.location(),
                )
            )
    ...
    def endElementNS(self, name, qname):
        ...
        element = Element(
            namespace_context=ns_ctx,
            expanded_name=this_state.expandedName(),
            attributes=this_state.__attributes,
            location=this_state.location(),
        )
...
class Node(xml.dom.Node, pyxb.utils.utility.Locatable_mixin):
    ...
    def _indexInParent(self):
        return self.__indexInParent
    ...
    nextSibling = property(
        lambda _s: _s.parentNode.__childIfPresent(_s.__indexInParent + 1)
    )
    ...
    def getAttributeNodeNS(self, ns_uri, local_name):
        return self.__attributes._getAttr(
            pyxb.namespace.ExpandedName(ns_uri, local_name)
        )
...
class Comment(_CharacterData):
    def __init__(self, text, **kw):
        super(Comment, self).__init__(
            value=text, node_type=xml.dom.Node.COMMENT_NODE, **kw
        )
