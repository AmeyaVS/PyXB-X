...
# %{?<cond>??<true>?:<false>?}
# %{?1 == 2??true?:false?}
_substConditionalPattern = re.compile(
    r"%{\?(?P<expr>.+?)\?\?(?P<true>.*?)(\?:(?P<false>.*?))?\?}",
...
_substIfDefinedPattern = re.compile(
    r"%{\?(?P<id>\w+)(\?\+(?P<repl>.*?))?(\?\-(?P<ndrepl>.*?))?\?}",
    re.MULTILINE + re.DOTALL,
)
...
def replaceInText(text, **dictionary):
    ...
    global _substDefinedBodyPattern
    rv = text
    rv = _substIfDefinedPattern.sub(
        lambda _x: _bodyIfDefinedPattern(_x, dictionary), rv
    )
    rv = _substConditionalPattern.sub(
        lambda _x: _bodyConditionalPattern(_x, dictionary), rv
    )
    rv = _substIdPattern.sub(
        lambda _x, _map=dictionary: _map.get(
            _x.group("id"), "%%{MISSING:%s}" % (_x.group("id"),)
        ),
        rv,
    )
    return rv
