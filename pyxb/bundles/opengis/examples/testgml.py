...
grid = gml.RectifiedGrid(
    limits, "latitude longitude", origin, offset_vector, dimension=2
)
grid.id = "_%x" % id(grid)
domain = gml.domainSet(grid)

val_template = gmlapp.Temperature(
    nilReason="template", _nil=True, uom="urn:x-si:v1999:uom:degreesC"
)
...
bds = pyxb.utils.domutils.BindingDOMSupport(
    namespace_prefix_map={gml.Namespace: "gml", gmlapp.Namespace: "app"}
)
