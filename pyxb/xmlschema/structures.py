...
class _SchemaComponent_mixin(
    ...
    def _namespaceContext(self):
        ...
        if self.__namespaceContext is None:
            raise pyxb.LogicError(
                "Attempt to access missing namespace context for %s" % (self,)
            )
    ...
    def __init__(self, *args, **kw):
        ...
        if self.__namespaceContext is None:
            if node is None:
                raise pyxb.LogicError(
                    "Schema component constructor must be given namespace_context or node"
                )
            self.__namespaceContext = pyxb.namespace.NamespaceContext.GetNodeContext(
                node
            )
        if self.__namespaceContext is None:
        ...
        if schema is not None:
            ...
            assert isinstance(
                self, (Schema, Annotation, Wildcard)
            ), "No origin available for type %s" % (type(self),)
    ...
    def _setOwner(self, owner):
        ...
        if owner is not None:
            assert (self.__owner is None) or (
                self.__owner == owner
            ), "Owner was %s set to %s" % (self.__owner, owner)
            self.__owner = owner
    ...
    def _resetClone_csc(self, **kw):
        ...
        self.__owner = owner
        assert not (
            isinstance(self, ComplexTypeDefinition) and isinstance(owner, Schema)
        )
        self.__ownedComponents = set()
        self.__clones = None
        ...
        return getattr(super(_SchemaComponent_mixin, self), "_resetClone_csc", self_fn)(
            **kw
        )
    ...
    def hasBinding(self):
        ...
        bindings."""
        return self.isTypeDefinition() or (
            isinstance(self, ElementDeclaration) and self._scopeIsGlobal()
        )
    ...
    def _updateFromOther_csc(self, other):
        ...
        assert self != other
        self_fn = lambda *_args, **_kw: self
        getattr(super(_SchemaComponent_mixin, self), "_updateFromOther_csc", self_fn)(
            other
        )
    ...
    def __new__(cls, *args, **kw):
        ...
        if not (singleton_property in cls.__dict__):
            setattr(
                cls,
                singleton_property,
                super(_Singleton_mixin, cls).__new__(cls, *args, **kw),
            )
...
class _PickledAnonymousReference(object):
    ...
    def __lookupObject(self):
        return self.__namespace.categoryMap(self.__AnonymousCategory).get(
            self.__anonymousName
        )
    ...
    def __str__(self):
        ...
        return "ANONYMOUS:%s" % (
            pyxb.namespace.ExpandedName(self.__namespace, self.__anonymousName),
        )
...
class _NamedComponent_mixin(pyxb.utils.utility.PrivateTransient_mixin, pyxb.cscRoot):
    ...
    def _setAnonymousName(self, namespace, unique_id=None, anon_name=None):
        ...
        if anon_name is None:
            ...
            anon_name = pyxb.utils.utility.MakeUnique(
                anon_name,
                set(six.iterkeys(namespace.categoryMap(self.__AnonymousCategory))),
            )
    ...
    def _anonymousName(self, namespace=None):
        assert (
            self.__anonymousName is not None
        ), "%x %s %s in %s missing anonymous name" % (
            id(self),
            type(self),
            self.name(),
    ...
    def __needAnonymousSupport(self):
        ...
        # name for it.
        return self.isAnonymous() or (
            self._scopeIsIndeterminate()
            and not isinstance(self, (AttributeGroupDefinition, ModelGroupDefinition))
        )
    ...
    def _prepareForArchive_csc(self, module_record):
        if self.__needAnonymousSupport():
            self._setAnonymousName(
                module_record.namespace(), unique_id=module_record.generationUID()
            )
        self_fn = lambda *_args, **_kw: self
        return getattr(
            super(_NamedComponent_mixin, self), "_prepareForArchive_csc", self_fn
        )(module_record)
    ...
    def _picklesInArchive(self, archive):
        ...
        assert not (
            self.targetNamespace() is None
        ), "%s has no tns, scope %s, location %s, schema %s" % (
            self,
            self._scope(),
            self._location(),
            self._schema().targetNamespace(),
        )
        ...
        if isinstance(scope, (tuple, _PickledAnonymousReference)):
            ...
            if scope_ctd is None:
                raise pyxb.SchemaValidationError(
                    "Unable to resolve local scope %s" % (scope_ref,)
                )
            ...
            if rv is None:
                raise pyxb.SchemaValidationError(
                    "Unable to resolve %s as %s in scope %s"
                    % (object_reference, icls, scope_ref)
                )
        elif _ScopedDeclaration_mixin.ScopeIsGlobal(
            scope
        ) or _ScopedDeclaration_mixin.ScopeIsIndeterminate(scope):
            if issubclass(icls, SimpleTypeDefinition) or issubclass(
                icls, ComplexTypeDefinition
            ):
            ...
            if rv is None:
                raise pyxb.SchemaValidationError(
                    "Unable to resolve %s as %s" % (object_reference, icls)
                )
        if rv is None:
            raise pyxb.SchemaValidationError(
                "Unable to resolve reference %s, scope %s ns %s type %s, class %s"
                % (
                    object_reference,
                    scope,
                    (scope is None and "<unknown>" or scope.targetNamespace()),
                    type(scope),
                    icls,
                )
            )
    ...
    def __init__(self, *args, **kw):
        ...
        # unqualified).
        self.__targetNamespace = kw.get(
            "target_namespace", self._namespaceContext().targetNamespace()
        )
    ...
    def isNameEquivalent(self, other):
        ...
        return (self == other) or (
            (not self.isAnonymous()) and (self.expandedName() == other.expandedName())
        )
    ...
    def _picklingReference(self):
        if self.__needAnonymousSupport():
            ...
            return _PickledAnonymousReference(
                self.targetNamespace(), self._anonymousName()
            )
    ...
    def __getnewargs__(self):
        ...
        if self.__pickleAsReference():
            ...
            if isinstance(self, _ScopedDeclaration_mixin):
                ...
                assert isinstance(self, _NamedComponent_mixin), (
                    "Pickling unnamed component %s in indeterminate scope by reference"
                    % (self,)
                )
                assert not isinstance(scope, ComplexTypeDefinition), "%s %s %s %s" % (
                    self,
                    self.name(),
                    scope,
                    self._objectOrigin(),
    ...
    def _resetClone_csc(self, **kw):
        self.__schema = None
        self_fn = lambda *_args, **_kw: self
        rv = getattr(super(_NamedComponent_mixin, self), "_resetClone_csc", self_fn)(
            **kw
        )
        self.__templateMap = {}
...
class _ValueConstraint_mixin(pyxb.cscRoot):
    ...
    def _valueConstraintFromDOM(self, node):
        ...
        if (adefault is not None) and (afixed is not None):
            raise pyxb.SchemaValidationError(
                "Attributes default and fixed may not both appear (3.2.3r1)"
            )
        if adefault is not None:
            if (ause is not None) and ("optional" != ause):
                raise pyxb.SchemaValidationError(
                    "Attribute use must be optional when default present (3.2.3r2)"
                )
...
class _ScopedDeclaration_mixin(pyxb.cscRoot):
    ...
    XSCOPE_indeterminate = (
        "indeterminate"  # <<< Marker to indicate scope has not been assigned
    )
    ...
    def _scopeIsCompatible(self, scope):
        ...
        if self.scope() == scope:
            return True
        return (self.SCOPE_global == self.scope()) and isinstance(
            scope, ComplexTypeDefinition
        )
...
class _AttributeWildcard_mixin(pyxb.cscRoot):
    ...
    def _attributeRelevantChildren(self, node_list):
        ...
        for node in node_list:
            ...
            if xsd.nodeIsNamed(node, "attribute"):
                ...
                if agd_en is None:
                    raise pyxb.SchemaValidationError(
                        "Require ref attribute on internal attributeGroup elements"
                    )
                attribute_groups.append(agd_en)
            elif xsd.nodeIsNamed(node, "anyAttribute"):
                if any_attribute is not None:
                    raise pyxb.SchemaValidationError(
                        "Multiple anyAttribute children are not allowed"
                    )
                any_attribute = node
    ...
    def CompleteWildcard(cls, namespace_context, attribute_groups, local_wildcard):
        ...
        if local_wildcard is not None:
            # Clause 2.2.1
            return Wildcard(
                process_contents=local_wildcard.processContents(),
                namespace_constraint=Wildcard.IntensionalIntersection(
                    agd_constraints + [local_wildcard.namespaecConstraint()]
                ),
        ...
        # Clause 2.2.2
        return Wildcard(
            process_contents=agd_wildcards[0].processContents(),
...
class AttributeDeclaration(
    ...
    def CreateFromDOM(cls, node, **kw):
        ...
        scope = kw["scope"]
        assert _ScopedDeclaration_mixin.ScopeIsIndeterminate(
            scope
        ) or _ScopedDeclaration_mixin.IsValidScope(scope)
        ...
        if xsd.nodeIsNamed(node.parentNode, "schema"):
            ...
            assert _ScopedDeclaration_mixin.ScopeIsIndeterminate(scope) or isinstance(
                scope, ComplexTypeDefinition
            )
        else:
            raise pyxb.SchemaValidationError(
                "Internal attribute declaration by reference"
            )
    ...
    def _resolve(self):
        ...
        assert (
            self.__typeExpandedName is not None
        ), "AD %s is unresolved but had no type attribute field" % (
            self.expandedName(),
        )
        self.__typeDefinition = self.__typeExpandedName.typeDefinition()
        if self.__typeDefinition is None:
            raise pyxb.SchemaValidationError(
                "Type reference %s cannot be found" % (self.__typeExpandedName,)
            )
        if not isinstance(self.__typeDefinition, SimpleTypeDefinition):
            raise pyxb.SchemaValidationError(
                "Need %s to be a simple type" % (self.__typeExpandedName,)
            )
...
class AttributeUse(
    ...
    def CreateFromDOM(cls, node, **kw):
        ...
        if use is not None:
            if "required" == use:
                ...
                raise pyxb.SchemaValidationError(
                    "Unexpected value %s for attribute use attribute" % (use,)
                )
        ...
        if rv.__refExpandedName is None:
            ...
            kw.pop("node", None)
            kw["owner"] = rv
            kw["target_namespace"] = schema.targetNamespaceForNode(
    ...
    def _resolve(self):
        ...
        if self.__attributeDeclaration is None:
            raise pyxb.SchemaValidationError(
                "Attribute declaration %s cannot be found" % (self.__refExpandedName,)
            )
...
class ElementDeclaration(
        ...
        if (type_definition is not None) and (self.valueConstraint() is not None):
            ...
            if isinstance(self.__typeDefinition, SimpleTypeDefinition):
                ...
                if ct is None:
                    ...
                    failed = not (
                        isinstance(ct, tuple)
                        and (ComplexTypeDefinition.CT_SIMPLE == ct[0])
                    )
            if failed:
                if self.__typeExpandedName is None:
                    raise pyxb.SchemaValidationError(
                        "Value constraint on element %s with non-simple content"
                        % (self.expandedName(),)
                    )
                raise pyxb.SchemaValidationError(
                    "Value constraint on element %s with non-simple type %s"
                    % (self.expandedName(), self.__typeExpandedName)
                )
        return self
    ...
    _SGE_Map = {"extension": SGE_extension, "restriction": SGE_restriction}
    ...
    def CreateFromDOM(cls, node, **kw):
        ...
        rv.__substitutionGroupExpandedName = domutils.NodeAttributeQName(
        ...
        for cn in node.childNodes:
            if (Node.ELEMENT_NODE == cn.nodeType) and xsd.nodeIsNamed(
                cn, "key", "unique", "keyref"
            ):
                identity_constraints.append(
                    IdentityConstraintDefinition.CreateFromDOM(cn, **kw)
                )
        ...
        if rv.__typeExpandedName is not None:
            if (simpleType_node is not None) and (complexType_node is not None):
                raise pyxb.SchemaValidationError(
                    "Cannot combine type attribute with simpleType or complexType child"
                )
        if (rv.__typeDefinition is None) and (simpleType_node is not None):
            rv._typeDefinition(
                SimpleTypeDefinition.CreateFromDOM(simpleType_node, **kw)
            )
        if (rv.__typeDefinition is None) and (complexType_node is not None):
            rv._typeDefinition(
                ComplexTypeDefinition.CreateFromDOM(complexType_node, **kw)
            )
        if rv.__typeDefinition is None:
            if rv.__typeExpandedName is None:
                ...
                for cn in node.childNodes:
                    if Particle.IsParticleNode(cn):
                        raise pyxb.SchemaValidationError(
                            "Node %s in element must be wrapped by complexType."
                            % (cn.localName,)
                        )
                rv._typeDefinition(ComplexTypeDefinition.UrTypeDefinition())
        rv.__isResolved = (rv.__typeDefinition is not None) and (
            rv.__substitutionGroupExpandedName is None
        )
    ...
    def isAdaptable(self, ctd):
        ...
        # can be....
        _log.warning(
            "Require %s to be resolved; might be a loop.", self.typeDefinition()
        )
        return False
    ...
    def _resolve(self):
        ...
        if self.__substitutionGroupExpandedName is not None:
            ...
            if sga is None:
                raise pyxb.SchemaValidationError(
                    "Element declaration refers to unrecognized substitution group %s"
                    % (self.__substitutionGroupExpandedName,)
                )
        ...
        if self.__typeDefinition is None:
            ...
            if td is None:
                raise pyxb.SchemaValidationError(
                    "Type declaration %s cannot be found" % (self.__typeExpandedName,)
                )
...
class ComplexTypeDefinition(
    ...
    _DM_Map = {"extension": DM_extension, "restriction": DM_restriction}
    ...
    def localScopedDeclarations(self, reset=False):
        ...
        if reset or (self.__localScopedDeclarations is None):
            rve = [
                _ed
                for _ed in six.itervalues(self.__scopedElementDeclarations)
                if (self == _ed.scope())
            ]
            rve.sort(key=lambda _x: _x.expandedName())
            rva = [
                _ad
                for _ad in six.itervalues(self.__scopedAttributeDeclarations)
                if (self == _ad.scope())
            ]
    ...
    def _recordLocalDeclaration(self, decl):
        ...
        if isinstance(decl, ElementDeclaration):
            ...
            raise pyxb.LogicError(
                "Unexpected instance of %s recording as local declaration"
                % (type(decl),)
            )
        ...
        if decl != existing_decl:
            if isinstance(decl, ElementDeclaration):
                ...
                if not pending_type.isDerivationConsistent(existing_type):
                    raise pyxb.SchemaValidationError(
                        "Conflicting element declarations for %s: existing %s versus new %s"
                        % (decl.expandedName(), existing_type, pending_type)
                    )
                ...
                if (existing_type != pending_type) and (
                    self.DM_restriction == self.derivationMethod()
                ):
                ...
                raise pyxb.SchemaValidationError(
                    "Multiple attribute declarations for %s" % (decl.expandedName(),)
                )
            else:
    ...
    CT_MIXED = (
        "MIXED"  # <<< Children may be elements or other (e.g., character) content
    )
    ...
    def UrTypeDefinition(cls, schema=None, in_builtin_definition=False):
        ...
        if cls.__UrTypeDefinition is None:
            ...
            bi._setAttributeWildcard(
                Wildcard(
                    namespace_constraint=Wildcard.NC_any,
                    process_contents=Wildcard.PC_lax,
                    **kw
                )
            )
    ...
    def CreateFromDOM(cls, node, **kw):
        ...
        if rv._scopeIsGlobal():
            ...
            if rv.isAnonymous():
                raise pyxb.SchemaValidationError(
                    "Anonymous complex type at schema top level"
                )
        else:
            assert not isinstance(rv.owner(), Schema)
            if not rv.isAnonymous():
                raise pyxb.SchemaValidationError(
                    "Name attribute invalid on non-global complex types: %s"
                    % (rv.expandedName(),)
                )
    ...
    def __completeProcessing(self, method, content_style):
        ...
        if self.__usesC1C2 is None:
            ...
            for ag_en in self.__attributeGroupNames:
                ...
                if agd is None:
                    raise pyxb.SchemaValidationError(
                        "Attribute group %s cannot be found" % (ag_en,)
                    )
                if not agd.isResolved():
                    self._queueForResolution(
                        "unresolved attribute group", depends_on=agd
                    )
                    return self
            ...
            for au in uses_c1c2:
                ...
                if not ad.isResolved():
                    ...
                    self._queueForResolution(
                        "unresolved attribute declaration %s from base type" % (ad_en,),
                        depends_on=ad,
                    )
                    return self
        ...
        if isinstance(self.__baseTypeDefinition, ComplexTypeDefinition):
            ...
            for au in uses_c3:
                if not au.isResolved():
                    self._queueForResolution(
                        "unresolved attribute use from base type", depends_on=au
                    )
                    return self
                ad = au.attributeDeclaration()
                if not ad.isResolved():
            ...
            if self.DM_restriction == method:
                ...
                for au in self.__usesC1C2:
                    ...
                    assert 1 >= len(
                        matching_uses
                    ), "Multiple inherited attribute uses with name %s"
        ...
        for au in self.__usesC1C2.union(uses_c3):
            ...
            if ad_en in use_map:
                raise pyxb.SchemaValidationError(
                    "Multiple definitions for %s in CTD %s"
                    % (ad_en, self.expandedName())
                )
            use_map[ad_en] = au
        ...
        if not self._scopeIsIndeterminate():
            for au in self.__attributeUses:
                assert (
                    not au.attributeDeclaration()._scopeIsIndeterminate()
                ), "indeterminate scope for %s" % (au,)
        ...
        # Clause 2
        complete_wildcard = _AttributeWildcard_mixin.CompleteWildcard(
            self._namespaceContext(), self.__attributeGroups, local_wildcard
        )
        ...
        if self.DM_restriction == method:
            ...
            if base_wildcard is not None:
                if complete_wildcard is None:
                    ...
                    # 3.2.2.1.2
                    self._setAttributeWildcard(
                        Wildcard(
                            process_contents=complete_wildcard.processContents(),
                            namespace_constraint=Wildcard.IntensionalUnion(
                                [
                                    complete_wildcard.namespaceConstraint(),
    ...
    def __simpleContent(self, method, **kw):
        ...
        if isinstance(self.__baseTypeDefinition, ComplexTypeDefinition):
            ...
            if (
                (type(parent_content_type) == tuple)
    ...
    def __setComplexContentFromDOM(
        self, type_node, content_node, definition_node_list, method, **kw
    ):
        # Do content type.  Cache the keywords that need to be used
        # for newly created schema components.
        ckw = kw.copy()
        ckw["namespace_context"] = pyxb.namespace.NamespaceContext.GetNodeContext(
            type_node
        )
        ...
        for cn in definition_node_list:
            ...
            if Particle.IsTypedefNode(cn):
                typedef_node = cn
                test_2_1_1 = False
            if xsd.nodeIsNamed(cn, "all", "sequence") and (
                not domutils.HasNonAnnotationChild(cn)
            ):
                test_2_1_2 = True
            if xsd.nodeIsNamed(cn, "choice") and (
                not domutils.HasNonAnnotationChild(cn)
            ):
                mo_attr = domutils.NodeAttribute(cn, "minOccurs")
                if (mo_attr is not None) and (0 == datatypes.integer(mo_attr)):
                    test_2_1_3 = True
    ...
    def __complexContent(self, method):
        ...
        if self.DM_restriction == method:
            ...
            if self.CT_EMPTY == self.__effectiveContent:
                ...
                self.__scopedElementDeclarations.update(
                    self.__baseTypeDefinition.__scopedElementDeclarations
                )
        else:
            # Clause 3.2
            assert self.DM_extension == method
            ...
            if self.CT_EMPTY == self.__effectiveContent:
                ...
                content_type = (
                    ct,
                    self.__effectiveContent,
                )  # ASSIGN EXTENSION LOCAL ONLY
            else:
                assert type(parent_content_type) == tuple
                m = ModelGroup(
                    compositor=ModelGroup.C_SEQUENCE,
                    particles=[parent_content_type[1], self.__effectiveContent],
                    **ckw
                )
                content_type = (
                    ct,
                    Particle(m, **ckw),
                )  # ASSIGN EXTENSION PARENT AND LOCAL

        assert (self.CT_EMPTY == content_type) or (
            (type(content_type) == tuple) and (content_type[1] is not None)
        )
        return content_type
    ...
    def __setContentFromDOM(self, node, **kw):
        ...
        if first_elt:
            ...
            if xsd.nodeIsNamed(first_elt, "simpleContent"):
                ...
                # the children
                if not Particle.IsParticleNode(
                    first_elt, "attributeGroup", "attribute", "anyAttribute"
                ):
                    raise pyxb.SchemaValidationError(
                        "Unexpected element %s at root of complexType"
                        % (first_elt.nodeName,)
                    )
            if have_content:
                # Repeat the search to verify that only the one child is present.
                content_node = domutils.LocateFirstChildElement(
                    node, require_unique=True
                )
                ...
                if xsd.nodeIsNamed(ions, "restriction"):
                    ...
                    if not is_complex_content:
                        ...
                        if ions_st is not None:
                            clause2_std = SimpleTypeDefinition.CreateFromDOM(
                                ions_st, **kw
                            )
                    ...
                    raise pyxb.SchemaValidationError(
                        "Expected restriction or extension as sole child of %s in %s"
                        % (content_node.nodeName, self.name())
                    )
                self.__baseExpandedName = domutils.NodeAttributeQName(ions, "base")
                if self.__baseExpandedName is None:
                    raise pyxb.SchemaValidationError(
                        "Element %s missing base attribute" % (ions.nodeName,)
                    )
        ...
        (attributes, attribute_group_names, any_attribute) = (
            self._attributeRelevantChildren(definition_node_list)
        )
        self.__usesC1 = set()
        ...
        if self.__isComplexContent:
            self.__setComplexContentFromDOM(
                node, content_node, definition_node_list, self.__derivationMethod, **kw
            )
    ...
    def _resolve(self):
        ...
        if self.__baseTypeDefinition is None:
            ...
            if base_type is None:
                raise pyxb.SchemaValidationError(
                    "Cannot locate %s: need import?" % (self.__baseExpandedName,)
                )
            if not base_type.isResolved():
                # Have to delay resolution until the type this
                # depends on is available.
                self._queueForResolution(
                    "unresolved base type %s" % (self.__baseExpandedName,),
                    depends_on=base_type,
                )
                return self
        ...
        # context.
        if isinstance(self.__contentType, tuple) and isinstance(
            self.__contentType[1], Particle
        ):
            prt = self.__contentType[1]
            if not prt.isAdaptable(self):
                self._queueForResolution(
                    "content particle %s is not deep-resolved" % (prt,)
                )
                return self
...
class AttributeGroupDefinition(
    ...
    def CreateFromDOM(cls, node, **kw):
        ...
        (attributes, attribute_group_names, any_attribute) = (
            rv._attributeRelevantChildren(node.childNodes)
        )
    ...
    def _resolve(self):
        ...
        if self.__anyAttribute is not None:
            local_wildcard = Wildcard.CreateFromDOM(self.__anyAttribute)
        self._setAttributeWildcard(
            _AttributeWildcard_mixin.CompleteWildcard(
                self._namespaceContext(), attribute_groups, local_wildcard
            )
        )
...
class ModelGroupDefinition(
    _SchemaComponent_mixin, _NamedComponent_mixin, _Annotated_mixin
):
    ...
    def CreateFromDOM(cls, node, **kw):
        ...
        for cn in node.childNodes:
            ...
            if ModelGroup.IsGroupMemberNode(cn):
                ...
                rv.__modelGroup = ModelGroup.CreateFromDOM(
                    cn, model_group_definition=rv, **kw
                )
...
class ModelGroup(_ParticleTree_mixin, _SchemaComponent_mixin, _Annotated_mixin):
            ...
            if Particle.IsParticleNode(cn):
                ...
                if 0 == particle.maxOccurs():
                    if getattr(cn, "_location", False):
                        ...
                        location = ""
                    _log.warning(
                        "Particle %s%s discarded due to maxOccurs 0"
                        % (particle, location)
                    )
                else:
                ...
                raise pyxb.SchemaValidationError(
                    "Unexpected element %s in model group" % (cn.nodeName,)
                )
    ...
    def __str__(self):
        ...
        return "%s:(%s)" % (
            comp,
            six.u(",").join([six.text_type(_p) for _p in self.particles()]),
        )
...
class Particle(
    ...
    def __init__(self, term, *args, **kw):
        ...
        assert "scope" in kw
        assert (self._scopeIsIndeterminate()) or isinstance(
            self._scope(), ComplexTypeDefinition
        )
        ...
        if self.__maxOccurs is not None:
            if self.__minOccurs > self.__maxOccurs:
                raise pyxb.LogicError(
                    "Particle minOccurs %s is greater than maxOccurs %s on creation"
    ...
    def _resolve(self):
        ...
        if ModelGroup == self.__resolvableType:
            ...
            if group_decl is None:
                raise pyxb.SchemaValidationError(
                    "Model group reference %s cannot be found"
                    % (self.__refExpandedName,)
                )
            ...
            if self.__refExpandedName is not None:
                ...
                if self.__pendingTerm is None:
                    raise pyxb.SchemaValidationError(
                        "Unable to locate element referenced by %s"
                        % (self.__refExpandedName,)
                    )
            assert self.__pendingTerm is not None
    ...
    def CreateFromDOM(cls, node, **kw):
        ...
        kw.update({"min_occurs": 1, "max_occurs": 1, "node": node})

        if not Particle.IsParticleNode(node):
            raise pyxb.LogicError(
                "Attempted to create particle from illegal element %s"
                % (node.nodeName,)
            )
        ...
        if xsd.nodeIsNamed(node, "group"):
            ...
            if rv.__refExpandedName is None:
                schema = kw.get("schema")
                assert schema is not None
                target_namespace = schema.targetNamespaceForNode(
                ...
                for tag in ("nillable", "default", "fixed", "form", "block", "type"):
                    ...
                    if av is not None:
                        raise pyxb.SchemaValidationError(
                            'element with "ref" cannot have "%s"' % (tag,)
                        )
            ...
            raise pyxb.LogicError(
                "Unhandled node in Particle.CreateFromDOM: %s" % (node.toxml("utf-8"),)
            )
    ...
    def IsParticleNode(cls, node, *others):
        return xsd.nodeIsNamed(
            node, "group", "all", "choice", "sequence", "element", "any", *others
        )
    ...
    def IntensionalUnion(cls, constraints):
        ...
        while 0 < len(constraints):
            ...
            if negated_ns is not None:
                ...
                if None in c_set:
                    raise pyxb.SchemaValidationError(
                        "Union of wildcard namespace constraints not expressible"
                    )
                o1 = c_tuple
                continue
            # 6
        ...
        while 0 < len(constraints):
            ...
            if isinstance(o1, tuple) and isinstance(o2, tuple):
                ...
                if (ns1 is not None) and (ns2 is not None) and (ns1 != ns2):
                    raise pyxb.SchemaValidationError(
                        "Intersection of wildcard namespace constraints not expressible"
                    )
                # 6
            ...
            if cls.NC_any == nc:
                ...
                for ns_uri in nc.split():
                    if cls.NC_local == ns_uri:
                        ...
                        ncs.add(
                            pyxb.namespace.NamespaceForURI(
                                ns_uri, create_if_missing=True
                            )
                        )
...
class IdentityConstraintDefinition(
    ...
    def CreateFromDOM(cls, node, **kw):
        ...
        if xsd.nodeIsNamed(node, "key"):
            ...
            if rv.__referAttribute is None:
                raise pyxb.SchemaValidationError(
                    "Require refer attribute on keyref elements"
                )
            ...
            raise pyxb.LogicError(
                "Unexpected identity constraint node %s" % (node.toxml("utf-8"),)
            )
        rv.__icc = icc
        ...
        for cn in domutils.LocateMatchingChildren(node, "field"):
            ...
            if xp_attr is None:
                raise pyxb.SchemaValidationError(
                    "field element missing xpath attribute"
                )
        ...
        if rv.ICC_KEYREF != rv.__identityConstraintCategory:
            rv._namespaceContext().targetNamespace().addCategoryObject(
                "identityConstraintDefinition", rv.name(), rv
            )
    ...
    def _resolve(self):
        ...
        if self.ICC_KEYREF == icc:
            ...
            if refer is None:
                self._queueForResolution(
                    "Identity constraint definition %s cannot be found" % (refer_en,),
                    depends_on=refer,
                )
                return self
...
class Annotation(_SchemaComponent_mixin):
    ...
    def __init__(self, **kw):
        ...
        if (application_information is not None) and (
            not isinstance(application_information, list)
        ):
    ...
    def asDocString(self):
        ...
        rv = self.text()
        rv = self.__RemoveMultiQuote_re.sub(
            lambda _mo: "'" * (_mo.end(0) - _mo.start(0)), rv
        )
        if rv.startswith('"'):
            rv = " " + rv
        if rv.endswith('"'):
...
class SimpleTypeDefinition(
    ...
    def primitiveTypeDefinition(self, throw_if_absent=True):
        if throw_if_absent:
            ...
            if self.__primitiveTypeDefinition is None:
                raise pyxb.LogicError(
                    "Expected primitive type for %s in %s", self, self.targetNamespace()
                )
    ...
    def __setstate__(self, state):
        ...
        """
        super_fn = getattr(
            super(SimpleTypeDefinition, self),
            "__setstate__",
            lambda _state: self.__dict__.update(_state),
        )
        super_fn(state)
    ...
    def __str__(self):
        ...
        if self.VARIETY_absent == self.variety():
            ...
            elts.append(
                ...
                % (
                    six.u(" ").join(
                        [
                            six.text_type(_mtd.name())
                            for _mtd in self.memberTypeDefinitions()
                        ],
                    )
                )
            )
        ...
        if self.__fundamentalFacets:
            elts.append("\n  ")
            elts.append(
                six.u(",").join([six.text_type(_f) for _f in self.__fundamentalFacets])
            )
    ...
    def _updateFromOther_csc(self, other):
        ...
        # The other STD should be an unresolved schema-defined type.
        assert other.__baseTypeDefinition is None, "Update from resolved STD %s" % (
            other,
        )
    ...
    def _CreateXMLInstance(cls, name, schema):
        ...
        if "space" == name:
            ...
            bi.__primitiveTypeDefinition = (
                bi.__baseTypeDefinition.__primitiveTypeDefinition
            )
            ...
            raise pyxb.IncompleteImplementationError(
                "No implementation for xml:%s" % (name,)
            )
        bi.__facets = {}
            ...
            if Node.ELEMENT_NODE == cn.nodeType:
                if not xsd.nodeIsNamed(cn, "simpleType"):
                    if other_elts_ok:
                        continue
                    raise pyxb.SchemaValidationError(
                        "Context requires element to be xs:simpleType"
                    )
                assert not simple_type_child
                simple_type_child = cn
        if simple_type_child is None:
            raise pyxb.SchemaValidationError(
                "Content requires an xs:simpleType member (or a base attribute)"
            )
    ...
    def __initializeFromList(self, body, **kw):
        ...
        if self.__itemTypeExpandedName is None:
            ...
            # that.
            self.__itemTypeDefinition = self.CreateFromDOM(
                self.__singleSimpleTypeChild(body), **kw
            )
    ...
    def __initializeFromRestriction(self, body, **kw):
        if self.__baseTypeDefinition is None:
            ...
            if self.__baseExpandedName is None:
                self.__baseTypeDefinition = self.CreateFromDOM(
                    self.__singleSimpleTypeChild(body, other_elts_ok=True), **kw
                )
    ...
    def __initializeFromUnion(self, body, **kw):
        ...
        if mta is not None:
            ...
            self.__memberTypesExpandedNames = [
                nsc.interpretQName(_mten) for _mten in mta.split()
            ]
        ...
        for ai in app_info:
            for cn in ai.childNodes:
                ...
                if pyxb.namespace.XMLSchema_hfp.nodeIsNamed(cn, "hasFacet"):
                    facet_name = domutils.NodeAttribute(
                        cn, "name"
                    )  # , pyxb.namespace.XMLSchema_hfp)
                    if facet_name is None:
                        raise pyxb.SchemaValidationError(
                            "hasFacet missing name attribute in %s" % (cn,)
                        )
                    if facet_name in seen_facets:
                        raise pyxb.SchemaValidationError(
                            "Multiple hasFacet specifications for %s" % (facet_name,)
                        )
                ...
                if pyxb.namespace.XMLSchema_hfp.nodeIsNamed(cn, "hasProperty"):
                    fundamental_facets.add(
                        facets.FundamentalFacet.CreateFromDOM(cn, self)
                    )
        if 0 < len(facet_map):
    ...
    def __updateFacets(self, body):
        ...
        for fc in facets.Facet.Facets:
            ...
            if 0 < len(children):
                    ...
                    base_type_definition=self.__baseTypeDefinition,
                ...
                for cn in children:
                    ...
                    try:
                        ...
                        raise pyxb.SchemaValidationError(
                            "Error assigning facet %s in %s: %s"
                            % (fc.Name(), self.expandedName(), e)
                        )
                local_facets[fc] = fi
    ...
    def _createRestriction(self, owner, body):
        ...
        std = SimpleTypeDefinition(
            owner=owner,
            namespace_context=owner._namespaceContext(),
            variety=None,
            scope=self._scope(),
    ...
    def __completeResolution(self, body, variety, alternative):
        ...
        if not self.__baseTypeDefinition.isResolved():
            self._queueForResolution(
                "base type %s is not resolved" % (self.__baseTypeDefinition,),
                depends_on=self.__baseTypeDefinition,
            )
            return self
        if variety is None:
        ...
        if self.VARIETY_absent == variety:
            ...
            ptd = self
            while isinstance(ptd, SimpleTypeDefinition) and (
                self.VARIETY_atomic == ptd.__baseTypeDefinition.variety()
            ):
            ...
            if self._DA_list == alternative:
                if self.__itemTypeExpandedName is not None:
                    self.__itemTypeDefinition = (
                        self.__itemTypeExpandedName.typeDefinition()
                    )
                    if not isinstance(self.__itemTypeDefinition, SimpleTypeDefinition):
                        raise pyxb.SchemaValidationError(
                            "Unable to locate STD %s for items"
                            % (self.__itemTypeExpandedName,)
                        )
            elif self._DA_restriction == alternative:
                self.__itemTypeDefinition = (
                    self.__baseTypeDefinition.__itemTypeDefinition
                )
            else:
                raise pyxb.LogicError(
                    "completeResolution list variety with alternative %s"
                    % (alternative,)
                )
            ...
            if self._DA_union == alternative:
                ...
                if self.__memberTypeDefinitions is None:
                    ...
                    if self.__memberTypesExpandedNames is not None:
                        for mn_en in self.__memberTypesExpandedNames:
                            ...
                            if std is None:
                                raise pyxb.SchemaValidationError(
                                    "Unable to locate member type %s" % (mn_en,)
                                )
                ...
                for mt in self.__memberTypeDefinitions:
                    ...
                    if not mt.isResolved():
                        self._queueForResolution(
                            "member type not resolved", depends_on=mt
                        )
                        return self
                ...
                raise pyxb.LogicError(
                    "completeResolution union variety with alternative %s"
                    % (alternative,)
                )
            # Save a unique copy
            self.__memberTypeDefinitions = mtd[:]
        ...
        try:
            pyxb.namespace.NamespaceContext.PushContext(
                pyxb.namespace.NamespaceContext.GetNodeContext(body)
            )
    ...
    def _resolve(self):
        ...
        kw = {"owner": self, "schema": self._schema()}
        ...
        if self.__baseTypeDefinition is None:
            raise pyxb.SchemaValidationError(
                "xs:simpleType must have list, union, or restriction as child"
            )
        ...
        if bad_instance:
            raise pyxb.SchemaValidationError(
                "Expected exactly one of list, restriction, union as child of simpleType"
            )
...
class _ImportElementInformationItem(_Annotated_mixin):
    ...
    def __init__(self, importing_schema, node, **kw):
        ...
        if uri is None:
            raise pyxb.IncompleteImplementationError(
                "import statements without namespace not supported"
            )
        schema_location = pyxb.utils.utility.NormalizeLocation(
            domutils.NodeAttribute(node, "schemaLocation"), importing_schema.location()
        )
        self.__schemaLocation = schema_location
        ns = self.__namespace = pyxb.namespace.NamespaceForURI(
        ...
        if self.schemaLocation() is not None:
            # @todo: NOTICE
            (has_schema, schema_instance) = self.__namespace.lookupSchemaByLocation(
                schema_location
            )
            if not has_schema:
                ckw = {
                    "absolute_schema_location": schema_location,
                ...
                try:
                    ...
                    _log.exception(
                        "Import %s cannot read schema location %s (%s)",
                        ns,
                        self.__schemaLocation,
                        schema_location,
                    )
                    raise
        ...
        if self.__schema is not None:
            if ns != self.__schema.targetNamespace():
                raise pyxb.SchemaValidationError(
                    "Import expected namespace %s but got %s"
                    % (ns, self.__schema.targetNamespace())
                )
...
class Schema(_SchemaComponent_mixin):
    ...
    __attributeMap = {
        pyxb.namespace.ExpandedName(None, "attributeFormDefault"): _UNQUALIFIED,
    ...
    def __init__(self, *args, **kw):
        ...
        if not isinstance(self.__targetNamespace, pyxb.namespace.Namespace):
            raise pyxb.LogicError(
                "Schema constructor requires valid Namespace instance as target_namespace"
            )
        ...
        self.__referencedNamespaces = list(
            six.itervalues(self._namespaceContext().inScopeNamespaces())
        )
    ...
    def CreateFromDOM(
        ...
        **kw
        ...
        assert (namespace_context is None) or isinstance(
            namespace_context, pyxb.namespace.NamespaceContext
        )
        ns_ctx = pyxb.namespace.NamespaceContext.GetNodeContext(
            root_node,
        ...
        if tns is None:
            raise pyxb.SchemaValidationError(
                "No targetNamespace associated with content (not a schema?)"
            )
        schema = cls(
            namespace_context=ns_ctx,
        ...
        if schema.targetNamespace() != ns_ctx.targetNamespace():
            raise pyxb.SchemaValidationError(
                "targetNamespace %s conflicts with %s"
                % (schema.targetNamespace(), ns_ctx.targetNamespace())
            )
        ...
        if not xsd.nodeIsNamed(root_node, "schema"):
            raise pyxb.SchemaValidationError(
                "Root node %s of document is not an XML schema element"
                % (root_node.nodeName,)
            )
    ...
    def targetNamespaceForNode(self, dom_node, declaration_type):
        ...
        if form_type is None:
            if declaration_type == ElementDeclaration:
                ...
                raise pyxb.LogicError(
                    "Expected ElementDeclaration or AttributeDeclaration: got %s"
                    % (declaration_type,)
                )
        tns = None
        if self._QUALIFIED == form_type:
            ...
            if self._UNQUALIFIED != form_type:
                raise pyxb.SchemaValidationError(
                    "Form type neither %s nor %s" % (self._QUALIFIED, self._UNQUALIFIED)
                )
        return tns
    ...
    def __processInclude(self, node):
        ...
        abs_uri = pyxb.utils.utility.NormalizeLocation(
            domutils.NodeAttribute(node, "schemaLocation"), self.__location
        )
        (has_schema, schema_instance) = self.targetNamespace().lookupSchemaByLocation(
            abs_uri
        )
        if not has_schema:
            kw = {
                "absolute_schema_location": abs_uri,
            ...
            try:
                ...
                _log.warning(
                    "Skipping apparent redundant inclusion of %s defining %s (hash matches %s)",
                    e.schemaLocation(),
                    e.namespace(),
                    e.existingSchema().location(),
                )
        ...
        if schema_instance:
            if self.targetNamespace() != schema_instance.targetNamespace():
                raise pyxb.SchemaValidationError(
                    "Included namespace %s not consistent with including namespace %s"
                    % (schema_instance.targetNamespace(), self.targetNamespace())
                )
    ...
    def __processTopLevelNode(self, node):
        ...
        raise pyxb.SchemaValidationError(
            "Unexpected top-level element %s" % (node.nodeName,)
        )
    ...
    def _addNamedComponent(self, nc):
        ...
        if not isinstance(nc, _NamedComponent_mixin):
            raise pyxb.LogicError(
                "Attempt to add unnamed %s instance to dictionary" % (nc.__class__,)
            )
        if nc.isAnonymous():
            raise pyxb.LogicError(
                "Attempt to add anonymous component to dictionary: %s", (nc.__class__,)
            )
        ...
        assert False, "No support to record named component of type %s" % (
            nc.__class__,
        )
    ...
    def __addTypeDefinition(self, td):
        ...
        if (old_td is not None) and (old_td != td):
            if isinstance(td, ComplexTypeDefinition) != isinstance(
                old_td, ComplexTypeDefinition
            ):
                raise pyxb.SchemaValidationError(
                    "Name %s used for both simple and complex types" % (td.name(),)
                )

            if not old_td._allowUpdateFromOther(td):
                raise pyxb.SchemaValidationError(
                    "Attempt to re-define non-builtin type definition %s"
                    % (tns.createExpandedName(local_name),)
                )
    ...
    def __addAttributeDeclaration(self, ad):
        ...
        if (old_ad is not None) and (old_ad != ad):
            if not old_ad._allowUpdateFromOther(ad):
                raise pyxb.SchemaValidationError(
                    "Attempt to re-define non-builtin attribute declaration %s"
                    % (tns.createExpandedName(local_name),)
                )
    ...
    def __addAttributeGroupDefinition(self, agd):
        ...
        if (old_agd is not None) and (old_agd != agd):
            if not old_agd._allowUpdateFromOther(agd):
                raise pyxb.SchemaValidationError(
                    "Attempt to re-define non-builtin attribute group definition %s"
                    % (tns.createExpandedName(local_name),)
                )
...
def _AddSimpleTypes(namespace):
    ...
    schema = Schema(
        namespace_context=pyxb.namespace.XMLSchema.initialNamespaceContext(),
        schema_location="URN:noLocation:PyXB:XMLSchema",
        generation_uid=pyxb.namespace.BuiltInObjectUID,
        _bypass_preload=True,
    )
    td = schema._addNamedComponent(
        ComplexTypeDefinition.UrTypeDefinition(schema, in_builtin_definition=True)
    )
    assert td.isResolved()
    # Add the simple ur type
    td = schema._addNamedComponent(
        SimpleTypeDefinition.SimpleUrTypeDefinition(schema, in_builtin_definition=True)
    )
    assert td.isResolved()
    ...
    for dtc in datatypes._PrimitiveDatatypes:
        ...
        td = schema._addNamedComponent(
            SimpleTypeDefinition.CreatePrimitiveInstance(name, schema, dtc)
        )
        assert td.isResolved()
    ...
    for dtc in datatypes._DerivedDatatypes:
        ...
        td = schema._addNamedComponent(
            SimpleTypeDefinition.CreateDerivedInstance(name, schema, parent_std, dtc)
        )
        assert td.isResolved()
        assert dtc.SimpleTypeDefinition() == td
        pts_std_map.setdefault(dtc, td)
    for dtc in datatypes._ListDatatypes:
        ...
        td = schema._addNamedComponent(
            SimpleTypeDefinition.CreateListInstance(list_name, schema, element_std, dtc)
        )
        assert td.isResolved()
